...
(function($){		
	$.fn.jqte = function(options){
		...
		return this.each(function(){
			...
			$(this).after('<div class="'+vars.css+'" style="height: '+vars.height+'px; width: '+vars.width+'px;border-top-left-radius: 0px;border-top-right-radius: 0px;border-bottom-left-radius: 0px;border-bottom-right-radius: 0px;margin-top: 0px;margin-bottom: 0px;padding-right: 2px;padding-left: 1px;'+getBorderColor()+'"></div>');
			
			// jqte
			var jQTE = $(this).next('.'+vars.css);
			var addBreaks = "";
			if(vars.contenteditable == "true")
			{
				addBreaks = "<br><br><br>";
			}

			// insert toolbar in jqte editor
			jQTE.html('<div id="'+vars.toolbarId+'" class="'+vars.css+"_toolbar"+'" role="toolbar" style="display: none;" unselectable></div>'+addBreaks+'<div class="'+vars.css+'_linkform" style="display:none" role="dialog"></div><div id="editor" class="'+vars.css+"_editor"+'" style="padding-top: 0px;padding-left: 0px;padding-right: 0px;padding-bottom: 0px;height: '+(vars.height+20)+'px;'+getFontColor()+'"></div>');
			...
			editor.attr("contenteditable",vars.contenteditable).html(thisElementVal);
			...
			// For read-only text areas, color the text gray
			function getFontColor()
			{
				if(vars.contenteditable == "false")
				{
					return "color: gray;";
				}
			...
			// For read-only text areas, color the border gray
			function getBorderColor()
			{
				if(vars.contenteditable == "false")
				{
					return "border-top-color: lightgray; border-bottom-color: lightgray; border-left-color: lightgray; border-right-color: lightgray; ";
				}
			...
			function affectStyleAround(element,style)
			{
				...
				if(selectedTag && style==false)
				{
					...
					{						
						var style = selectedTag.parent().attr("style");
						selectedTag.attr("style",style);

						adjustTextStyle(selectedTag);
					}
			...
			function adjustTextStyle(selectedTag)
			{
				selectedTag.css("height","");
				selectedTag.css("width","");
				selectedTag.css("padding-top","");
				selectedTag.css("padding-bottom","");
				selectedTag.css("padding-left","");
				selectedTag.css("padding-right","");
				selectedTag.css("margin-top","");
				selectedTag.css("margin-bottom","");
				selectedTag.css("border-top-left-radius","");
				selectedTag.css("border-top-right-radius","");
				selectedTag.css("border-bottom-left-radius","");
				selectedTag.css("border-bottom-right-radius","");
			}
			...
			function selected2style(styleCommand)
			{
				if(!toolbar.data("sourceOpened"))
				{
					...
					styleField.find("a").unbind("click").click(function()
					{
						...
						replaceSelection("div","style",styleType+":"+styleValue+";"+prevStyles);
			...
			function extractToText(strings)
			{
				...
				if(!vars.p && !vars.br)
					$htmlContent = $htmlContent.replace(/\<p>(.*?)\<\/p>/ig, '$1');
			...
			editor
				...
				.bind("keypress keyup keydown drop cut copy DOMCharacterDataModified DOMSubtreeModified",function()
				{
					// export contents of the text to the sources
					if(!toolbar.data("sourceOpened"))
						$(this).trigger("change");
				...
				.mouseout(function()
				{
					// if the mouseout method is added run the mouseout method   
					if($.isFunction(vars.mouseout))
						vars.mouseout();
				});
