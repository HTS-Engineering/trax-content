import { $ as shouldMockEndpoint, D as shouldSimulateError, a0 as createAxiosError, a1 as ENDPOINT_REGEX, v as validateFile, g as generateFileId, f as fileToBase64, a as addUploadedFile, Y as pickReconciliationScenario, Z as RECONCILIATION_SUCCESS_SUMMARY, h as hasUploadedFile, b as getUploadedFile, c as base64ToBuffer, d as deleteUploadedFile, a2 as getBusinessPurposes, a3 as addBusinessPurpose, a4 as updateBusinessPurpose, a5 as deleteBusinessPurpose, i as isMileageRequest, j as createExpenseDraftResponse, s as setRawRequest, k as addMileageDraft, l as buildMileageDraftMock, m as addExpenseDraft, n as buildExpenseDraftMock, o as findDraftById, e as generateBusinessId, r as reverseMapToMileageFormData, u as updateMileageDraft, p as reverseMapToExpenseFormData, q as updateExpenseDraft, y as deleteDraftById, z as addMileageSubmitted, A as buildSubmittedMileageMock, B as addExpenseSubmitted, C as buildSubmittedExpenseMock, t as getRawRequest, w as getExpenseDrafts, T as getExpenseSubmitted, U as mapMockToExpenseFormResponse, W as findItemById, x as getMileageDrafts, V as getMileageSubmitted, G as mockLogicalCompanies, H as mockFormTypes, I as getExpenseTypes, K as addExpenseType, L as updateExpenseType, M as getExpenseType, N as getMileageRates, a6 as generateDefaultMileageRates, R as setMileageRates, O as recalculateAllRateStatuses, P as sortRatesByStatus, S as calculateRateStatus, X as buildReconciliationListResponse, _ as STATEMENT_ACTION_SUCCESS, J as getNextExpenseTypeId, Q as getNextMileageRateId } from "./reconciliation-mock-u5MmNC-x.js";
import { d as formatToISODate } from "./date-format-5v64ZhEr.js";
import { E as EXPENSE_ENDPOINTS, z as MileageRateStatus } from "./expense-api-3lZYq6fv.js";
import { E as ExpenseFormStatus } from "./expense-data-Du2WjKhi.js";
import { b as devWarn, a as devLog } from "./index-DGIiGjWN.js";
class AxiosStrategy {
  active = false;
  interceptorId = null;
  axiosInstance;
  ...
  async initialize() {
    ...
    this.interceptorId = this.axiosInstance.interceptors.request.use(
      async (config) => {
        ...
        if (fullUrl.match(/\/api\/v1\.0\/expense-forms\/[^/]+\/[^/]+$/) && !fullUrl.includes("/drafts") && !fullUrl.includes("/cost-allocation") && config.method === "get") {
          return this.handleExpenseFormByIdMock(config, fullUrl);
        }
        if (fullUrl.match(/\/api\/v1\.0\/expense-forms\/[^/]+$/) && config.method === "get") {
          return this.handleExpenseFormsListMock(config, fullUrl);
        }
        if (fullUrl.match(/\/api\/v1\.0\/approvals\/[^/]+$/) && config.method === "get") {
          return this.handleApprovalsListMock(config, fullUrl);
        }
        if (fullUrl.match(/\/api\/v1\.0\/approvals\/[^/]+\/[^/]+\/approval-history$/) && config.method === "get") {
          return this.handleApprovalHistoryMock(config);
        }
        ...
        if (ENDPOINT_REGEX.RECONCILIATION_BANK_STATEMENTS.test(fullUrl) && config.method === "get") {
          return this.handleReconciliationListMock(config, fullUrl);
        }
        if (ENDPOINT_REGEX.RECONCILIATION_CSV_UPLOAD.test(fullUrl) && config.method === "post") {
          return this.handleReconciliationCsvUploadMock(config);
        }
        if (ENDPOINT_REGEX.RECONCILIATION_BANK_STATEMENT_RELEASE.test(fullUrl) && config.method === "post") {
          return this.handleReconciliationReleaseMock(config, fullUrl);
        }
        if (ENDPOINT_REGEX.RECONCILIATION_BANK_STATEMENT_CANCEL.test(fullUrl) && config.method === "delete") {
          return this.handleReconciliationCancelMock(config, fullUrl);
        }
        return config;
      },
  ...
  /**
   * Handle reconciliation list mock (GET /api/v1.0/reconciliation/bank-statements).
   */
  async handleReconciliationListMock(config, fullUrl) {
    await this.delay(300);
    const urlObj = new URL(fullUrl, "http://localhost");
    const pageNumber = Number(urlObj.searchParams.get("pageNumber") ?? "1");
    const pageSize = Number(urlObj.searchParams.get("pageSize") ?? "10");
    const mockResponse = {
      data: buildReconciliationListResponse({ pageNumber, pageSize }),
      status: 200,
      statusText: "OK",
      headers: {},
      config
  ...
  /** Handle reconciliation CSV upload mock. Scenario picked by file-name prefix. */
  async handleReconciliationCsvUploadMock(config) {
    await this.delay(1200);
    const formData = config.data;
    const file = formData == null ? void 0 : formData.get("file");
    const name = (file == null ? void 0 : file.name) ?? "";
    ...
    const mockResponse = {
      data: RECONCILIATION_SUCCESS_SUMMARY,
      status: 200,
      statusText: "OK",
      headers: {},
      config
  ...
  async handleReconciliationReleaseMock(config, fullUrl) {
    await this.delay(400);
    const match = fullUrl.match(/\/bank-statements\/([^/]+)\/release$/);
    const statementId = match ? decodeURIComponent(match[1]) : "unknown";
    const mockResponse = {
      data: STATEMENT_ACTION_SUCCESS("released", statementId),
      status: 200,
      statusText: "OK",
      headers: {},
      config
  ...
  async handleReconciliationCancelMock(config, fullUrl) {
    await this.delay(400);
    const match = fullUrl.match(/\/bank-statements\/([^/]+)\/cancel$/);
    const statementId = match ? decodeURIComponent(match[1]) : "unknown";
    const mockResponse = {
      data: STATEMENT_ACTION_SUCCESS("cancelled", statementId),
      status: 200,
      statusText: "OK",
      headers: {},
      config
  ...
  async handleMileageRateCreateMock(config, fullUrl) {
    ...
    const previousMonthEndDateOnly = formatToISODate(previousMonthEnd) ?? null;
    rates = rates.map((rate) => {
      const rateEffective = new Date(rate.effectiveDate);
      if (!rate.expiryDate && rateEffective < newEffectiveDate) {
        return { ...rate, expiryDate: previousMonthEndDateOnly };
      }
      return rate;
    });
    const newRate = {
  ...
  // ─── Approvals mock handlers ───────────────────────────────────────
  async handleApprovalsListMock(config, fullUrl) {
    await this.delay(300);
    const url = new URL(fullUrl, "http://localhost");
    const status = url.searchParams.get("status") || "submitted";
    const pageNumber = Number(url.searchParams.get("pageNumber") ?? 1);
    const pageSize = Number(url.searchParams.get("pageSize") ?? 10);
    const allItems = this.getMockApprovalItems();
    ...
    devLog("✅ Axios Interceptor: Approvals list mock", { status, count: paged.length });
    config.adapter = () => Promise.resolve(mockResponse);
    return config;
  }
  async handleExpenseFormByIdMock(config, fullUrl) {
    await this.delay(300);
    const match = fullUrl.match(/\/expense-forms\/[^/]+\/([^/?]+)/);
    const itemId = match == null ? void 0 : match[1];
    if (!itemId) {
      const mockResponse2 = {
        data: { error: "Invalid expense form ID" },
        status: 400,
        statusText: "Bad Request",
        headers: {},
        config
    ...
    const mockDetail = this.getMockExpenseFormDetail(itemId);
    if (mockDetail) {
      devLog("✅ Axios Interceptor: Expense form detail mock", { id: itemId });
      const mockResponse2 = {
        data: mockDetail,
        status: 200,
        statusText: "OK",
    ...
    devLog("❌ Axios Interceptor: Expense form not found", { id: itemId });
    const mockResponse = {
      data: { error: "Expense form not found" },
      status: 404,
      statusText: "Not Found",
      headers: {},
      config
  ...
  async handleApprovalHistoryMock(config) {
  ...
  /** Generate mock approval list items (ApprovalApiResponse format). */
  ...
  /** Generate mock SingleExpenseFormResponse for expense form detail. */
  getMockExpenseFormDetail(itemId) {
    const items = {
      "10001": {
        expenseId: 10001,
        status: "Submitted",
