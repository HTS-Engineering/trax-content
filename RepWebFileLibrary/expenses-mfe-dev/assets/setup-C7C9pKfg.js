...
import { D as DiagAPI, r as registerGlobal, g as getGlobal, u as unregisterGlobal, c as createContextKey, b as baggageEntryMetadataFromString, p as propagation, t as trace, i as isSpanContextValid, T as TraceFlags, a as context, S as SpanStatusCode, d as isValidTraceId, I as INVALID_SPAN_CONTEXT, R as ROOT_CONTEXT } from "./__federation_expose_Mount-cMT-mPNK.js";
...
const getPerformanceNavigationEntries = /* @__PURE__ */ __name(() => {
  var _a;
  const entries = {};
  const performanceNavigationTiming = (_a = otperformance.getEntriesByType) == null ? void 0 : _a.call(otperformance, "navigation")[0];
...
const addSpanPerformancePaintEvents = /* @__PURE__ */ __name((span) => {
  var _a;
  const performancePaintTiming = (_a = otperformance.getEntriesByType) == null ? void 0 : _a.call(otperformance, "paint");
...
const _DocumentLoadInstrumentation = class _DocumentLoadInstrumentation extends InstrumentationBase {
  ...
  _addResourcesSpans(rootSpan) {
    var _a;
    const resources = (_a = otperformance.getEntriesByType) == null ? void 0 : _a.call(otperformance, "resource");
    if (resources) {
  ...
  _collectPerformance() {
    ...
    context.with(propagation.extract(ROOT_CONTEXT, { traceparent }), () => {
      var _a;
      const rootSpan = this._startSpan(AttributeNames$2.DOCUMENT_LOAD, PerformanceTimingNames.FETCH_START, entries);
      if (!rootSpan) {
        return;
      }
      context.with(trace.setSpan(context.active(), rootSpan), () => {
        const fetchSpan = this._startSpan(AttributeNames$2.DOCUMENT_FETCH, PerformanceTimingNames.FETCH_START, entries);
        if (fetchSpan) {
          if (this._semconvStability & SemconvStability.OLD) {
            fetchSpan.setAttribute(ATTR_HTTP_URL$2, location.href);
          }
      ...
      if (this._semconvStability & SemconvStability.OLD) {
        rootSpan.setAttribute(ATTR_HTTP_URL$2, location.href);
        rootSpan.setAttribute(ATTR_HTTP_USER_AGENT$2, navigator.userAgent);
      }
      if (this._semconvStability & SemconvStability.STABLE) {
  ...
  _initResourceSpan(resource, parentSpan) {
    var _a;
    const span = this._startSpan(AttributeNames$2.RESOURCE_FETCH, PerformanceTimingNames.FETCH_START, resource, parentSpan);
    if (span) {
      if (this._semconvStability & SemconvStability.OLD) {
        span.setAttribute(ATTR_HTTP_URL$2, resource.name);
      }
      if (this._semconvStability & SemconvStability.STABLE) {
...
__name(_DocumentLoadInstrumentation, "DocumentLoadInstrumentation");
let DocumentLoadInstrumentation = _DocumentLoadInstrumentation;
...
function registerAutoInstrumentations(opts) {
  ...
  const instrumentations = [
    // fetch + traceparent header -> BE kore-logging links to the same trace.id.
    new FetchInstrumentation({ propagateTraceHeaderCorsUrls }),
    // Axios uses XHR, so this covers every API call without touching axiosInstance.ts.
    new XMLHttpRequestInstrumentation({ propagateTraceHeaderCorsUrls }),
    // Initial page load via PerformanceObserver; SPA routes -> use-trace-route.ts.
...
function buildResource() {
  return resourceFromAttributes({
    ...
    [ATTR_SERVICE_VERSION]: "0.1.0-dev.local-1779367419034",
...
function initializeObservability(config) {
  ...
  const exporter = new OTLPTraceExporter({ url: buildExporterUrl(config.collectorUrl) });
