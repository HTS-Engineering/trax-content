...
import { p as ECostAllocation, I as ItemCategory, l as ExpenseFormType, z as MileageRateStatus, A as AllowedMimeType, M as MIME_TYPE_CONFIG, F as FormTypeId } from "./expense-api-BVxEXUfY.js";
import "./index-vtzBEVcA.js";
import { E as ExpenseFormStatus } from "./expense-data-Du2WjKhi.js";
import { s as formatToISODate, t as parseDateOnlyAsLocal } from "./date-format-B1PByQhX.js";
const ENDPOINT_PATTERNS = {
  ...
  // Expense form by ID
  EXPENSE_FORM_BY_ID: "*/api/v1.0/expense-forms/:company/:id",
  // Approvals
  APPROVALS_LIST: "*/api/v1.0/approvals/:company",
  APPROVAL_HISTORY: "*/api/v1.0/approvals/:company/:expenseFormId/approval-history",
  // Reconciliation
  RECONCILIATION_BANK_STATEMENTS: "*/api/v1.0/reconciliation/bank-statements",
  RECONCILIATION_CSV_UPLOAD: "*/api/v1.0/reconciliation/:masterAccountId/csv-upload",
  RECONCILIATION_BANK_STATEMENT_RELEASE: "*/api/v1.0/reconciliation/bank-statements/:statementId/release",
  RECONCILIATION_BANK_STATEMENT_CANCEL: "*/api/v1.0/reconciliation/bank-statements/:statementId/cancel",
  RECONCILIATION_COMPANY_TRANSACTIONS_AGGREGATION: "*/api/v1.0/reconciliation/:statementId/transactions-company"
};
const ENDPOINT_REGEX = {
  ...
  CONFIG_LOGICAL_COMPANIES: /\/api\/v1\.0\/configuration\/logical-companies/,
  // Reconciliation (tolerates optional `?query` so MSW matches full URLs too)
  RECONCILIATION_BANK_STATEMENTS: /\/api\/v1\.0\/reconciliation\/bank-statements(\?.*)?$/,
  RECONCILIATION_CSV_UPLOAD: /\/api\/v1\.0\/reconciliation\/(?!bank-statements)[^/]+\/csv-upload(\?.*)?$/,
  RECONCILIATION_BANK_STATEMENT_RELEASE: /\/api\/v1\.0\/reconciliation\/bank-statements\/[^/]+\/release(\?.*)?$/,
  RECONCILIATION_BANK_STATEMENT_CANCEL: /\/api\/v1\.0\/reconciliation\/bank-statements\/[^/]+\/cancel(\?.*)?$/,
  RECONCILIATION_COMPANY_TRANSACTIONS_AGGREGATION: /\/api\/v1\.0\/reconciliation\/(?!bank-statements)[^/]+\/transactions-company(\?.*)?$/
};
const mockExpenseDrafts = [
  {
    ...
    data: {
      ...
      netAmount: "145.83",
      totalAmount: "175.00",
      netCurrency: { code: "CAD", symbol: "$" },
      totalCurrency: { code: "CAD", symbol: "$" },
...
__name(getExpenseDrafts, "getExpenseDrafts");
function getExpenseSubmitted() {
  return store.expenseSubmitted;
}
__name(getExpenseSubmitted, "getExpenseSubmitted");
function getMileageDrafts() {
  return store.mileageDrafts;
}
__name(getMileageDrafts, "getMileageDrafts");
function getMileageSubmitted() {
  return store.mileageSubmitted;
}
__name(getMileageSubmitted, "getMileageSubmitted");
...
__name(findDraftById, "findDraftById");
...
__name(getBusinessPurposes, "getBusinessPurposes");
...
__name(addBusinessPurpose, "addBusinessPurpose");
...
__name(updateBusinessPurpose, "updateBusinessPurpose");
...
__name(deleteBusinessPurpose, "deleteBusinessPurpose");
...
__name(addExpenseType, "addExpenseType");
...
__name(updateExpenseType, "updateExpenseType");
...
__name(getMileageRates, "getMileageRates");
function setMileageRates(companyShortName, expenseTypeId, rates) {
  const key = getMileageRatesKey(companyShortName, expenseTypeId);
  mileageRatesStore.set(key, rates);
}
__name(setMileageRates, "setMileageRates");
...
__name(generateDefaultMileageRates, "generateDefaultMileageRates");
...
const ACTIVE_MOCKS = {
  RECONCILIATION_BANK_STATEMENTS: false,
  RECONCILIATION_CSV_UPLOAD: false,
  RECONCILIATION_BANK_STATEMENT_RELEASE: false,
  RECONCILIATION_BANK_STATEMENT_CANCEL: false,
  RECONCILIATION_COMPANY_TRANSACTIONS_AGGREGATION: false,
  CONFIG_LOGICAL_COMPANIES: false
};
const ENDPOINT_META = {
  RECONCILIATION_BANK_STATEMENTS: {
    regex: ENDPOINT_REGEX.RECONCILIATION_BANK_STATEMENTS,
    description: "Reconciliation bank-statements list (dashboard fixture)"
  },
  RECONCILIATION_CSV_UPLOAD: {
    regex: ENDPOINT_REGEX.RECONCILIATION_CSV_UPLOAD,
    description: "Reconciliation CSV upload (filename-prefix scenario routing)"
  },
  RECONCILIATION_BANK_STATEMENT_RELEASE: {
    regex: ENDPOINT_REGEX.RECONCILIATION_BANK_STATEMENT_RELEASE,
    description: "Reconciliation statement release (always 200)"
  },
  RECONCILIATION_BANK_STATEMENT_CANCEL: {
    regex: ENDPOINT_REGEX.RECONCILIATION_BANK_STATEMENT_CANCEL,
    description: "Reconciliation statement cancel (always 200)"
  },
  RECONCILIATION_COMPANY_TRANSACTIONS_AGGREGATION: {
    regex: ENDPOINT_REGEX.RECONCILIATION_COMPANY_TRANSACTIONS_AGGREGATION,
    description: "Company-level transactions aggregation per statement (TRX-435)"
  },
  CONFIG_LOGICAL_COMPANIES: {
    regex: ENDPOINT_REGEX.CONFIG_LOGICAL_COMPANIES,
    description: "Logical companies list (sidebar / company switcher fallback)"
  }
};
const MOCK_KEYS = Object.keys(ACTIVE_MOCKS);
function shouldMockEndpoint(url) {
  for (const key of MOCK_KEYS) {
    if (ACTIVE_MOCKS[key] && ENDPOINT_META[key].regex.test(url)) {
      return true;
    }
  }
  return false;
}
__name(shouldMockEndpoint, "shouldMockEndpoint");
function getEndpointErrorProbability(endpoint) {
  for (const key of MOCK_KEYS) {
    if (ACTIVE_MOCKS[key] && ENDPOINT_META[key].regex.test(endpoint)) {
      return ENDPOINT_META[key].errorProbability;
    }
  }
  return void 0;
}
__name(getEndpointErrorProbability, "getEndpointErrorProbability");
const DEFAULT_CONFIG = {
  enabled: true,
  globalProbability: 20,
  endpointOverrides: {}
};
const getErrorConfig = /* @__PURE__ */ __name(() => {
...
const selectRandomError = /* @__PURE__ */ __name(() => {
...
const shouldSimulateError = /* @__PURE__ */ __name((endpoint) => {
  var _a;
...
const generateMockError = /* @__PURE__ */ __name((_endpoint) => {
...
const createAxiosError = /* @__PURE__ */ __name((endpoint, config) => {
...
__name(generateBusinessId, "generateBusinessId");
function createExpenseDraftResponse(id, businessId, data, isNew, existingCreatedDate) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const today = formatToISODate(/* @__PURE__ */ new Date()) ?? "";
  return {
    id,
    businessId,
    ...
    periodEnd: data.periodEnd ?? today,
...
__name(createExpenseDraftResponse, "createExpenseDraftResponse");
function reverseMapToMileageFormData(data) {
  ...
  return {
    ...
    formType: "period",
    expensePeriod: {
      from: data.periodStart ? parseDateOnlyAsLocal(data.periodStart) : /* @__PURE__ */ new Date(),
      to: data.periodEnd ? parseDateOnlyAsLocal(data.periodEnd) : /* @__PURE__ */ new Date()
    }
  };
}
__name(reverseMapToMileageFormData, "reverseMapToMileageFormData");
...
__name(reverseMapToExpenseFormData, "reverseMapToExpenseFormData");
function isMileageRequest(data) {
  return data.formTypeId === FormTypeId.MILEAGE;
}
__name(isMileageRequest, "isMileageRequest");
...
__name(buildMileageDraftMock, "buildMileageDraftMock");
...
__name(buildExpenseDraftMock, "buildExpenseDraftMock");
...
__name(buildSubmittedMileageMock, "buildSubmittedMileageMock");
...
__name(buildSubmittedExpenseMock, "buildSubmittedExpenseMock");
...
__name(calculateRateStatus, "calculateRateStatus");
...
__name(recalculateAllRateStatuses, "recalculateAllRateStatuses");
...
__name(sortRatesByStatus, "sortRatesByStatus");
const mapMockToExpenseFormResponse = /* @__PURE__ */ __name((item, index) => {
  var _a;
...
}, "mapMockToExpenseFormResponse");
export {
  addBusinessPurpose as $,
  ...
  mockLogicalCompanies as G,
  mockFormTypes as H,
  getExpenseTypes as I,
  getNextExpenseTypeId as J,
  addExpenseType as K,
  updateExpenseType as L,
  getExpenseType as M,
  getMileageRates as N,
  recalculateAllRateStatuses as O,
  sortRatesByStatus as P,
  getNextMileageRateId as Q,
  setMileageRates as R,
  calculateRateStatus as S,
  getExpenseSubmitted as T,
  mapMockToExpenseFormResponse as U,
  getMileageSubmitted as V,
  findItemById as W,
  shouldMockEndpoint as X,
  createAxiosError as Y,
  ENDPOINT_REGEX as Z,
  getBusinessPurposes as _,
  addUploadedFile as a,
  updateBusinessPurpose as a0,
  deleteBusinessPurpose as a1,
  generateDefaultMileageRates as a2,
