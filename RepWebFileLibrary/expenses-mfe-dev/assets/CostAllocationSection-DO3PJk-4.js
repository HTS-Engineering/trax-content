...
import { bo as createLucideIcon, aB as formatDate, bH as Decimal, bI as devWarn, bJ as getCadTaxBaseAmount, v as jsxRuntimeExports, aa as Icon, bK as ETL_ERROR_MESSAGE, H as apiClient, bL as FILE_ENDPOINTS, W as devError, bM as Ss, bN as Cs, av as gr, bO as _s, aj as Yn, az as notifyMessage, bP as getErrorMessage, aq as Ue, bQ as RefreshCw, bu as isConvertedExpense, E as useCompanyStore, bi as useTaxTypesDisplay, bp as formatExpenseDate, bC as formatCurrency, aG as ExpenseFormType, bm as EMPTY_CURRENCY_SYMBOL, bd as ys, a6 as jr, bR as ECostAllocation, a0 as us, aX as notifyError, bw as useDefaultCurrency, bS as formatDistance, bB as formatRate, bG as formatExpensePeriod, bT as ERROR_MESSAGES, bU as DEFAULT_CURRENCY_CODE, b3 as formatToISODate, bt as useDefaultCompany, aE as useExpenseTypes, al as FormTypeId, aM as Ba, bV as create, bW as devtools, bX as AllowedMimeType, bY as FILE_SIZE_LIMITS, bZ as MIME_TYPE_CONFIG, b_ as FilePreviewType, b4 as parseDateOnlyAsLocal, bv as useCountries, b$ as useDefaultCountry, c0 as queryClient, G as queryKeys, aW as devLog, c1 as isHttpApiError, L as AppError, J as EXPENSE_ENDPOINTS, c2 as isNotFoundError, c3 as extractError, c4 as useDebouncedCallback, bA as useEffectiveMileageRate, aU as useCurrencies, by as useFormTypeId, au as getExpenseBaseAmount, $ as isRegularExpense, O as isMileageExpense, X as isExpenseItemSubmitted, bD as isExpenseItemDraft, c5 as ActionType, C as useQueryClient, F as useMutation, Q as Qs, c6 as ItemCategory, b2 as useQuery, c7 as MILEAGE_RATES_STALE_TIME, c8 as fetchEffectiveMileageRate, c9 as useExpenseFormHistory, ca as findLastSentBack, aA as useExpenseItem, cb as useNumericDisplay, cc as Qt, aY as gn, b5 as ts, cd as getCurrencySymbol, ar as Mt, as as Et, at as Pt, ce as Ws, cf as Ks, cg as Xs, bz as Ga, ap as Oa, ch as cr, ci as oi, cj as Ye, ck as qa } from "./__federation_expose_Mount-DvZfOK8R.js";
import { S as SIGNED_DECIMAL_FORMAT_REGEX, m as TAX_RATE_FORMAT_REGEX, s as string, o as object, d as custom, c as boolean, b as array, D as DECIMAL_FORMAT_REGEX, g as date, p as unknown, n as number, _ as _enum, C as ConfirmDialog, f as useWatch, u as useForm, a as u, l as literal, i as useFormState, q as TOOLTIP_DELAY_QUICK, e as Controller, h as createDecimalChangeHandler, T as TOOLTIP_DELAY_TRUNCATED_TEXT } from "./schemas-DxXQ4AZl.js";
import { importShared } from "./__federation_fn_import-CZ2UOLBn.js";
import { _ as __vitePreload } from "./preload-helper-Bsq79q8M.js";
import { P as Plus } from "./plus-C2vh1175.js";
const __iconNode = [
  [
    "path",
    {
...
const PreviewReceiptSection = /* @__PURE__ */ __name(({ receipt, stretchToFill = false }) => {
  ...
  const [loadedBlobUrl, setLoadedBlobUrl] = useState$9();
  const [isLoading, setIsLoading] = useState$9(false);
  const [loadError, setLoadError] = useState$9(null);
  const [retryCount, setRetryCount] = useState$9(0);
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-x-8", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          PreviewField,
          {
            className: "w-full",
            label: "Rate",
            value: data.ratePerUnit ? formatRate(data.ratePerUnit, data.rateUnit, { currency }) : void 0
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
...
const { useCallback: useCallback$o, useEffect: useEffect$k, useMemo: useMemo$k, useRef: useRef$g } = await importShared("react");
...
const validateCostAllocation = /* @__PURE__ */ __name((ctx, costAllocations, amount, defer, currencyCode) => {
  ...
  allocations.forEach((allocation, index) => {
    ...
    if (amountValue !== 0 && allocation.amount === 0) {
      ctx.addIssue({
        code: "custom",
...
const { useCallback: useCallback$m, useEffect: useEffect$j, useRef: useRef$f, useState: useState$7 } = React$2;
...
const { useEffect: useEffect$i, useRef: useRef$e } = await importShared("react");
...
const { Fragment, memo: memo$3, useCallback: useCallback$l, useContext, useMemo: useMemo$i, useState: useState$6 } = await importShared("react");
const FormSectionComponent = /* @__PURE__ */ __name(({
  ...
  const propsWithDisabled = useMemo$i(
    () => ({
      ...renderProps,
...
const { useCallback: useCallback$k, useEffect: useEffect$g, useMemo: useMemo$g, useRef: useRef$c } = await importShared("react");
...
function useBaseExpenseForm(config, props) {
  ...
  const costAllocationsDirty = useMemo$g(() => {
  ...
  const watchedReceipt = useWatch({
    control,
    name: ExpenseFormField.ReceiptAttachment
  });
  const watchedSupportingFiles = useWatch({
    control,
    name: ExpenseFormField.SupportingFiles
  });
  const filesDirty = useMemo$g(() => {
    ...
    return JSON.stringify(watchedReceipt ?? null) !== JSON.stringify(baselineReceipt ?? null) || JSON.stringify(watchedSupportingFiles ?? []) !== JSON.stringify(baselineSupporting ?? []);
  }, [watchedReceipt, watchedSupportingFiles, rhfDefaultValues]);
  ...
  const getSubmitButtonState = useCallback$k(() => {
    ...
    if (requiresChangesToSubmit && !isDirty && !costAllocationsDirty && !filesDirty) {
      return { disabled: true, tooltip: "No changes made" };
    }
...
const { useCallback: useCallback$j, useMemo: useMemo$e } = await importShared("react");
...
const { useCallback: useCallback$g, useEffect: useEffect$f, useMemo: useMemo$d } = await importShared("react");
...
const { useEffect: useEffect$e, useRef: useRef$a } = await importShared("react");
...
const { useCallback: useCallback$f, useMemo: useMemo$c } = await importShared("react");
...
const { useCallback: useCallback$e, useEffect: useEffect$c, useMemo: useMemo$b, useRef: useRef$9 } = await importShared("react");
const { useCallback: useCallback$d, useEffect: useEffect$b, useImperativeHandle, useRef: useRef$8 } = await importShared("react");
...
const { useEffect: useEffect$a, useRef: useRef$7 } = await importShared("react");
const useFormButtonStateSync = /* @__PURE__ */ __name(({
  ...
  }, [form.getSubmitButtonState, form.getSaveDraftButtonState]);
...
const { useEffect: useEffect$8, useMemo: useMemo$a } = await importShared("react");
const useReimbursableAmountSync = /* @__PURE__ */ __name(({
  ...
  const calculatedAmount = useMemo$a(() => {
    try {
...
const mileageTripFormSchema = object({
  ...
  if (data.mileageType && data.expenseDate && parseFloat(data.reimbursableAmount) === 0) {
    ctx.addIssue({
      code: "custom",
      message: ZERO_REIMBURSABLE_AMOUNT_MESSAGE,
      path: [MileageTripFormField.ReimbursableAmount]
    });
  }
});
...
const mileagePeriodFormSchema = object({
  ...
  if (data.mileageType && from && parseFloat(data.ratePerUnit || "0") <= 0) {
    ctx.addIssue({
      code: "custom",
      message: NO_MILEAGE_RATE_FOR_DATE_MESSAGE,
      path: [MileagePeriodFormField.ExpensePeriod]
  ...
  if (data.mileageType && from && parseFloat(data.reimbursableAmount) === 0) {
    ctx.addIssue({
      code: "custom",
      message: ZERO_REIMBURSABLE_AMOUNT_MESSAGE,
      path: [MileagePeriodFormField.ReimbursableAmount]
    });
  }
});
...
const { useCallback: useCallback$9, useMemo: useMemo$8 } = await importShared("react");
...
const { useCallback: useCallback$8, useMemo: useMemo$7 } = await importShared("react");
...
const { useEffect: useEffect$7, useMemo: useMemo$6 } = await importShared("react");
...
const useMileageRateSync = /* @__PURE__ */ __name(({
  ...
  useEffect$7(() => {
    ...
    const unit = selectedType.unitOfMeasurement ?? "";
    let wrote = false;
...
const useHomeCurrency = /* @__PURE__ */ __name(() => {
  ...
  const { userDefaultCountryIso } = useDefaultCountry();
  const { defaultCurrencyCode, defaultCurrencySymbol } = useDefaultCurrency();
  const homeCountryHasCurrency = useMemo$5(
    () => {
      var _a;
...
const useExpenseFormSync = /* @__PURE__ */ __name(({
  ...
  useEffect$5(() => {
    if (formType) {
      if (getValues("formType") !== formType) setValue("formType", formType);
      if (formType !== ExpenseFormType.ENTERTAINMENT) {
        trigger(ExpenseFormField.PersonsEntertained);
      }
    }
  }, [formType, setValue, getValues, trigger]);
  useEffect$5(() => {
...
const SENT_BACK_PREFIX = {
  [ActionType.Rejected]: "Rejected on",
  [ActionType.Cancelled]: "Cancelled on"
};
function getHeaderSubtitle(item, mode, sentBackFrom) {
  if (!item) return void 0;
  if (mode === "preview" && isExpenseItemSubmitted(item)) {
    return getExpenseActionSubtitle(item);
  }
  if (mode === "draft" && isExpenseItemDraft(item)) {
    const datePart = sentBackFrom ? `${SENT_BACK_PREFIX[sentBackFrom.actionType]} ${formatDate(sentBackFrom.actionTimestamp)}` : `Created on ${formatDate(item.createdAt)}`;
    return [item.businessId, datePart].filter(Boolean).join(" • ");
  }
  return void 0;
}
...
function useExpenseItemHeader(options) {
  const { expenseItem, mode, isNewItem, itemType, sentBackFrom } = options;
  ...
  const subtitle = useMemo$1(
    () => getHeaderSubtitle(expenseItem, mode, sentBackFrom),
    [expenseItem, mode, sentBackFrom]
  );
...
const useSubmitExpense = /* @__PURE__ */ __name(() => {
  ...
  return useMutation({
    ...
    onSuccess: /* @__PURE__ */ __name((_data, { draftId, isResubmission }) => {
      queryClient2.invalidateQueries({ queryKey: queryKeys.expenses.all() });
      queryClient2.invalidateQueries({ queryKey: queryKeys.expenseDrafts.all() });
      queryClient2.invalidateQueries({ queryKey: queryKeys.expensesList.all() });
      queryClient2.invalidateQueries({ queryKey: queryKeys.expenseHistory.all() });
      queryClient2.removeQueries({
        queryKey: queryKeys.expenseItem.detail(draftId)
      });
      Qs.success(isResubmission ? "Expense resubmitted" : "Expense submitted", {
        duration: 3e3
      });
    }, "onSuccess")
  });
...
const useSubmitMileage = /* @__PURE__ */ __name(() => {
  ...
  return useMutation({
    ...
    onSuccess: /* @__PURE__ */ __name((_data, { draftId, isResubmission }) => {
      ...
      Qs.success(
        isResubmission ? "Mileage claim resubmitted" : "Mileage claim submitted",
        { duration: 3e3 }
      );
    }, "onSuccess")
  });
...
function useExpenseItemMutations(options) {
  ...
  const handleExpenseSubmit = useCallback$4(async (data, options2) => {
    ...
    onDraftSaved == null ? void 0 : onDraftSaved(draftId);
    await submitExpenseMutation.mutateAsync({ draftId, isResubmission });
    ...
    onExit();
  }, [submitExpenseMutation, saveExpenseDraftMutation, currentDraftId, isResubmission, onExit, onAfterMutation, onDraftSaved]);
  const handleExpenseSaveDraft = useCallback$4(async (data, options2) => {
    ...
    try {
      ...
      onDraftSaved == null ? void 0 : onDraftSaved(draft.id);
      onAfterMutation == null ? void 0 : onAfterMutation("saved");
    } catch {
    }
  }, [saveExpenseDraftMutation, currentDraftId, onDraftCreated, onAfterMutation, onDraftSaved]);
  ...
  }, [saveExpenseDraftMutation, currentDraftId, onDraftCreated, onDraftSaved]);
  const handleMileageSubmit = useCallback$4(async (data, options2) => {
    ...
    onDraftSaved == null ? void 0 : onDraftSaved(draftId);
    await submitMileageMutation.mutateAsync({ draftId, isResubmission });
    if ((_a = options2 == null ? void 0 : options2.signal) == null ? void 0 : _a.aborted) return;
    onAfterMutation == null ? void 0 : onAfterMutation("submitted");
    onExit();
  }, [submitMileageMutation, saveMileageDraftMutation, currentDraftId, isResubmission, onExit, onAfterMutation, onDraftSaved]);
  ...
  }, [saveMileageDraftMutation, currentDraftId, onDraftCreated, onAfterMutation, onDraftSaved]);
  const handleMileageSaveDraftForUpload = useCallback$4(async (data) => {
    ...
    onDraftSaved == null ? void 0 : onDraftSaved(draft.id);
    return { draftId: draft.id };
  }, [saveMileageDraftMutation, currentDraftId, onDraftCreated, onDraftSaved]);
...
const KEY_PREFIX = "expenses:editedDraft:";
const thisPage = /* @__PURE__ */ new Set();
const keyFor = /* @__PURE__ */ __name((draftId) => `${KEY_PREFIX}${draftId}`, "keyFor");
function withStorage(operation) {
  try {
    return operation(localStorage);
  } catch (error) {
    devWarn("Edited-draft marker unavailable: local storage is blocked.", error);
    return void 0;
  }
}
__name(withStorage, "withStorage");
function markDraftEdited(draftId) {
  thisPage.add(draftId);
  withStorage((storage) => storage.setItem(keyFor(draftId), "1"));
}
__name(markDraftEdited, "markDraftEdited");
function clearDraftEdited(draftId) {
  thisPage.delete(draftId);
  withStorage((storage) => storage.removeItem(keyFor(draftId)));
}
__name(clearDraftEdited, "clearDraftEdited");
function isDraftEdited(draftId) {
  return thisPage.has(draftId) || withStorage((storage) => storage.getItem(keyFor(draftId)) !== null) === true;
}
__name(isDraftEdited, "isDraftEdited");
function useDraftConversion({
  itemId,
  isDraft,
  companyShortName
}) {
  const { data } = useExpenseFormHistory({
    expenseFormId: itemId ? Number(itemId) : 0,
    companyShortName,
    enabled: !!itemId
  });
  if (!isDraft || !data) return null;
  return findLastSentBack(data.history);
}
__name(useDraftConversion, "useDraftConversion");
...
function useExpenseItemForm(options) {
  ...
  const [savedItemId, setSavedItemId] = useState$3(null);
  const requiresChangesToSubmit = itemId !== null && !!sentBackFrom && savedItemId !== itemId && !isDraftEdited(itemId);
  const handleDraftSaved = useCallback$3(
    (draftId) => {
      if (sentBackFrom) markDraftEdited(draftId);
      setSavedItemId(draftId);
    },
    [sentBackFrom]
  );
  const handleAfterMutation = useCallback$3(
    (kind) => {
      if ((kind === "submitted" || kind === "deleted") && itemId) clearDraftEdited(itemId);
      onAfterMutation == null ? void 0 : onAfterMutation(kind);
    },
    [itemId, onAfterMutation]
  );
...
export {
  ...
  useCostAllocationHandlers as f,
  CostAllocationSection as g,
  costAllocationItemSchema as h,
  isMileageTripData as i,
  isValidFileAttachment as j,
  ExpenseFormField as k,
  useFormFieldValues as l,
  mapCostAllocation as m,
  computeMileageEffectiveOn as n,
