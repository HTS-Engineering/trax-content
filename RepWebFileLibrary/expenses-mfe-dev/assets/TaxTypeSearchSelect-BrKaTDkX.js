...
import { a8 as Subscribable, aa as notifyManager, aq as shallowEqualObjects, ar as replaceEqualDeep, am as QueryObserver, i as useQueryClient, as as useIsRestoring, at as useQueryErrorResetBoundary, au as ensureSuspenseTimers, av as ensurePreventErrorBoundaryRetry, aw as useClearResetErrorBoundary, ac as noop, ax as shouldSuspend, ay as fetchOptimistic, az as getHasError, X as DEFAULT_CURRENCY_CODE, a2 as getCurrencySymbol, F as formatToISODate, G as parseDateOnlyAsLocal, T as FILE_ENDPOINTS, C as useQuery, t as useAppQuery, q as queryKeys, k as EXPENSE_ENDPOINTS, j as useMutation } from "./use-scroll-into-view-ref-BYTXl063.js";
import { h as create, i as devtools, s as subscribeWithSelector, j as immer, a as apiClient, C as CONFIGURATION_ENDPOINTS, k as devWarn, d as devError, A as gn } from "./configuration-Cg_hJZZz.js";
...
function isSubmittedPreviewRenderable(item) {
  var _a2;
  if (!isExpenseItemSubmitted(item)) return true;
  const data = item.data;
  return hasText(data.expenseDescription) || hasText(data.businessPurpose) || hasText(data.expenseDate) || Boolean((_a2 = data.expensePeriod) == null ? void 0 : _a2.from);
}
__name(isSubmittedPreviewRenderable, "isSubmittedPreviewRenderable");
...
const useCreateExpenseType = /* @__PURE__ */ __name(() => {
  const queryClient = useQueryClient();
  return useMutation({
    meta: { errorCopy: { fallback: "Could not create the expense type. Please try again." } },
...
const useUpdateExpenseType = /* @__PURE__ */ __name(() => {
  const queryClient = useQueryClient();
  return useMutation({
    meta: { errorCopy: { fallback: "Could not save the expense type. Please try again." } },
...
const useToggleExpenseTypeStatus = /* @__PURE__ */ __name(() => {
  const queryClient = useQueryClient();
  return useMutation({
    meta: { errorCopy: { fallback: "Could not change the expense type status. Please try again." } },
    mutationFn: /* @__PURE__ */ __name(async ({
      companyShortName,
      id,
      isActive
...
const useCreateMileageRate = /* @__PURE__ */ __name(() => {
  const queryClient = useQueryClient();
  return useMutation({
    meta: { errorCopy: { fallback: "Could not add the mileage rate. Please try again." } },
...
const useDeleteMileageRate = /* @__PURE__ */ __name(() => {
  const queryClient = useQueryClient();
  return useMutation({
    meta: { errorCopy: { fallback: "Could not delete the mileage rate. Please try again." } },
...
export {
  ...
  MIME_TYPE_CONFIG as B,
  FilePreviewType as C,
  useDefaultCountry as D,
  ExpenseTypeScope as E,
  FormTypeId as F,
  MILEAGE_RATES_STALE_TIME as G,
  fetchEffectiveMileageRate as H,
  ItemCategory as I,
  ...
  useExpenseItem as d,
  isSubmittedPreviewRenderable as e,
