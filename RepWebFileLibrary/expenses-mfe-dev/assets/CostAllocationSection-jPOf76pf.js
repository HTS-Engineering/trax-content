...
import { P as Plus } from "./plus-Dr5X6Frz.js";
const __iconNode = [
  [
    "path",
    {
...
const useCostAllocationHandlers = /* @__PURE__ */ __name((setValue, getValues, trigger, costAllocationsField, isEqualSplitField, getBaseAmount) => {
  ...
  const addAllocation = useCallback$n(
    (type) => {
  ...
  const updateAllocationEntity = useCallback$n(
    (id, entity) => {
      const currentAllocations = getValues(costAllocationsField) || [];
  ...
  const removeAllocation = useCallback$n(
    (id) => {
      const currentAllocations = getValues(costAllocationsField) || [];
      const isEqualSplit = isEqualSplitField ? getValues(isEqualSplitField) : false;
  ...
  const getSelectedValue = useCallback$n(
...
const { useCallback: useCallback$l, useRef: useRef$e } = await importShared("react");
...
const { useCallback: useCallback$f, useRef: useRef$9 } = await importShared("react");
...
const useExpenseFormSync = /* @__PURE__ */ __name(({
  ...
  const prevNetAmountRef = useRef$8(netAmount);
  const prevExpenseLocationRef = useRef$8(expenseLocation);
  const prevPaymentMethodRef = useRef$8(paymentMethod);
...
const { useCallback: useCallback$b, useMemo: useMemo$8 } = await importShared("react");
const useExpenseFormHandlers = /* @__PURE__ */ __name((setValue, getValues, trigger) => {
  const getBaseAmount = useCallback$b(() => {
...
const { useMemo: useMemo$7, useState: useState$7 } = await importShared("react");
const useExpenseFormLeftColumn = /* @__PURE__ */ __name(({
  ...
  const [isReceiptUploading, setIsReceiptUploading] = useState$7(false);
  ...
  const shouldShowReceiptCheckbox = useMemo$7(() => {
...
const { useCallback: useCallback$a, useMemo: useMemo$6 } = await importShared("react");
const useMileageTripFormHandlers = /* @__PURE__ */ __name((setValue, getValues, trigger) => {
  const getBaseAmount = useCallback$a(
...
const { useCallback: useCallback$9, useEffect: useEffect$7, useMemo: useMemo$5, useRef: useRef$7 } = await importShared("react");
function useBlobUrlManager(options = {}) {
  const { shouldKeepUrl } = options;
  const blobUrlsRef = useRef$7(/* @__PURE__ */ new Set());
  const abortControllerRef = useRef$7(null);
  const batchControllersRef = useRef$7(/* @__PURE__ */ new Set());
  const shouldKeepUrlRef = useRef$7(shouldKeepUrl);
  shouldKeepUrlRef.current = shouldKeepUrl;
  const trackUrl = useCallback$9((url) => {
    blobUrlsRef.current.add(url);
  }, []);
  const revokeUrl = useCallback$9((url) => {
  ...
  const isOwned = useCallback$9((url) => {
    return blobUrlsRef.current.has(url);
  }, []);
  const getAbortSignal = useCallback$9(() => {
    var _a;
    (_a = abortControllerRef.current) == null ? void 0 : _a.abort();
    abortControllerRef.current = new AbortController();
    return abortControllerRef.current.signal;
  }, []);
  const createBatchController = useCallback$9(() => {
    const controller = new AbortController();
...
function useFileOperations(options) {
  ...
  const validateFile = useCallback$8(
    (file) => {
      return validateReceiptFile(file);
    },
    []
  );
  const createAttachmentFromResponse = useCallback$8(
    (file, response) => {
  ...
  const uploadSingleFile = useCallback$8(
  ...
  const deleteFileById = useCallback$8(async (fileId) => {
    await deleteFile(fileId);
  }, []);
  const loadFileContent = useCallback$8(
    async (fileId, signal) => {
  ...
  const openPreview = useCallback$8(
    async (attachment) => {
      try {
...
const { useCallback: useCallback$7, useEffect: useEffect$6, useRef: useRef$6, useState: useState$6 } = await importShared("react");
function useDeleteDialog(options = {}) {
  const { onDeleteSuccess } = options;
  const [isOpen, setIsOpen] = useState$6(false);
  const [isDeleteFlowActive, setIsDeleteFlowActive] = useState$6(false);
  const shouldNavigateRef = useRef$6(false);
  useEffect$6(() => {
    return () => {
      shouldNavigateRef.current = false;
    };
  }, []);
  const open = useCallback$7(() => {
    setIsOpen(true);
  }, []);
  const close = useCallback$7(() => {
    setIsOpen(false);
  }, []);
  const handleOpenChange = useCallback$7((isOpen2) => {
    setIsOpen(isOpen2);
  }, []);
  const markForNavigation = useCallback$7(() => {
    shouldNavigateRef.current = true;
    setIsDeleteFlowActive(true);
    setIsOpen(false);
  }, []);
  const handleCloseComplete = useCallback$7(() => {
...
function useExpenseItemMutations(options) {
  ...
  const handleDeleteSuccess = useCallback$6(() => {
  ...
  const handleExpenseSubmit = useCallback$6(async (data, options2) => {
    var _a;
  ...
  const handleExpenseSaveDraft = useCallback$6(async (data, options2) => {
    var _a;
    try {
      const { draft } = await saveExpenseDraftMutation.mutateAsync({
        data,
  ...
  const handleExpenseSaveDraftForUpload = useCallback$6(async (data) => {
    const { draft } = await saveExpenseDraftMutation.mutateAsync({
      data,
      draftId: currentDraftId
    });
  ...
  const handleMileageSubmit = useCallback$6(async (data, options2) => {
    var _a;
    let draftId = currentDraftId;
    try {
      if (!draftId) {
  ...
  const handleMileageSaveDraft = useCallback$6(async (data, options2) => {
    var _a;
    try {
      const { draft } = await saveMileageDraftMutation.mutateAsync({
        data,
  ...
  const handleMileageSaveDraftForUpload = useCallback$6(async (data) => {
    const { draft } = await saveMileageDraftMutation.mutateAsync({
      data,
      draftId: currentDraftId
    });
  ...
  const handleDeleteConfirm = useCallback$6(() => {
...
const { useCallback: useCallback$5, useEffect: useEffect$5, useMemo: useMemo$3, useRef: useRef$5, useState: useState$5 } = await importShared("react");
...
function useExpenseItemForm(options) {
  ...
  const formRef = useRef$5(null);
  const abortControllerRef = useRef$5(new AbortController());
  const [submitButtonState, setSubmitButtonState] = useState$5({ disabled: true });
  const [saveDraftButtonState, setSaveDraftButtonState] = useState$5({ disabled: true });
  const [formKey, setFormKey] = useState$5(() => isNewItem ? "new" : `loading-${itemId}`);
  const {
  ...
  const handleDraftSaved = useCallback$5(
    (draftId) => {
  ...
  const handleAfterMutation = useCallback$5(
    (kind) => {
  ...
  const handleDeleteDraft = useCallback$5(() => {
  ...
  const handleDeleteDialogOpenChange = useCallback$5((open) => {
  ...
  const queryClient2 = useQueryClient();
  const handleOpenChange = useCallback$5((open) => {
    if (!open) {
...
const { useCallback: useCallback$4, useEffect: useEffect$4, useRef: useRef$4, useState: useState$4 } = React$2;
...
const ReceiptUpload = /* @__PURE__ */ __name(({
  ...
  const initialReceiptRef = useRef$4(initialReceipt);
  const attachmentRef = useRef$4(void 0);
  const {
    userDefaultCompany,
    blobManager,
    isStoreDataRelevant,
  ...
  const fileLoadAttemptedRef = useRef$4(/* @__PURE__ */ new Set());
  const handleFileUploadInternalRef = useRef$4(null);
  ...
  const prevAttachmentIdRef = useRef$4(void 0);
  useEffect$4(() => {
...
function ConditionalSection({
  ...
  const previousDisabledRef = useRef$3(void 0);
...
const { Fragment, memo: memo$3, useCallback: useCallback$3, useContext, useMemo: useMemo$1, useState: useState$3 } = await importShared("react");
