...
import { V as Subscribable, W as Mutation, X as notifyManager, Y as matchMutation, Z as noop, _ as hashQueryKeyByOptions, $ as Query, a0 as matchQuery, a1 as focusManager, a2 as onlineManager, a3 as resolveStaleTime, a4 as functionalUpdate, a5 as hashKey, a6 as partialMatchKey, a7 as skipToken, r as useQuery, q as queryKeys, f as useCompanyStore } from "./formatters-Dvm414Po.js";
import { b as apiClient, C as CONFIGURATION_ENDPOINTS, d as devError } from "./configuration-CaBH14y1.js";
import { b as isRetryableApiError } from "./api-error-Dja-8hfm.js";
...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      ...
      gcTime: 10 * 60 * 1e3,
      // Retry only transient failures (network/timeout/5xx), never a 4xx the server already decided
      retry: /* @__PURE__ */ __name((failureCount, error) => isRetryableApiError(error) ? failureCount < 3 : false, "retry"),
    ...
    mutations: {
      // Retry once on transient failures only; never re-send a mutation the server rejected with a 4xx
      retry: /* @__PURE__ */ __name((failureCount, error) => isRetryableApiError(error) ? failureCount < 1 : false, "retry"),
      retryDelay: 1e3
    }
  },
  queryCache: new QueryCache({
    // Deliberately no global 403 -> redirect here. The approvals list 403s an admin who lacks the
    // manager role, where the backend should instead return an empty 200; a blanket redirect would
    // eject a legitimate admin. Once the backend returns empty 200 for that case, a subpage 403 can
    // safely land the user on My Expenses from here (key off error.response?.status === 403).
