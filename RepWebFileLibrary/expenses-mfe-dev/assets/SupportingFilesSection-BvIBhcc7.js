...
import { s as string, o as object, b as boolean, f as custom, a as array, g as date, u as unknown, n as number, l as literal, c as useForm, d as a, e as useWatch, h as useFormState, C as Controller } from "./zod-0mrSG730.js";
import { importShared } from "./__federation_fn_import-DYyjX-5O.js";
import { j as jsxRuntimeExports } from "./jsx-runtime-TULtkvNU.js";
import { V as fs, X as ps, Z as hs, $ as vs, a0 as bs, a1 as xs, P as Pe, _ as _n, i as is, b as cs, l as ls, Y as Yn, a5 as Dt, a6 as ka, H as Ha, D as Da, N as Nn, F as Fn, I as In, a7 as Vs, a8 as ks, a9 as Ts, L as Ls } from "./index.es-DjMdAnbR.js";
import { I as Icon } from "./Icon-0rSrBywh.js";
import { E as ECostAllocation, F as FILE_ENDPOINTS } from "./cost-allocation-EVQYlSMW.js";
import { a as apiClient } from "./axiosInstance-DY_sVwZo.js";
import { c as useDebouncedCallback } from "./use-debounced-callback-BFqiFFta.js";
import { P as Plus } from "./plus-ZNfoAuFV.js";
import { _ as __vitePreload } from "./preload-helper-e_IRvegh.js";
const { useCallback: useCallback$g, useEffect: useEffect$d, useRef: useRef$a, useState: useState$5 } = await importShared("react");
...
const ConfirmDialog = ({
  ...
  const onCloseCompleteRef = useRef$a(onCloseComplete);
  const closeCompleteFiredRef = useRef$a(false);
  useEffect$d(() => {
  ...
  const handleAnimationEnd = useCallback$g(() => {
    var _a;
...
const { useCallback: useCallback$f, useEffect: useEffect$c, useMemo: useMemo$f, useRef: useRef$9 } = await importShared("react");
function useBaseExpenseForm(config2, props) {
  ...
  const handleDraftSave = useCallback$f(
    async () => {
      try {
  ...
  const validateForm = useCallback$f(async () => {
    const isFormValid = await trigger();
    return isFormValid && canSubmit;
  }, [trigger, canSubmit]);
  const getSubmitButtonState = useCallback$f(() => {
    if (isSubmitting) {
  ...
  const getSaveDraftButtonState = useCallback$f(() => {
    if (isDrafting) {
...
const { useCallback: useCallback$d, useMemo: useMemo$d } = await importShared("react");
const useCostAllocationHandlers = (setValue, getValues, costAllocationsField, isEqualSplitField, totalAmountField2) => {
  ...
  const addAllocation = useCallback$d(
    (type) => {
  ...
  const updateAllocationEntity = useCallback$d(
    (id, entity) => {
      const currentAllocations = getValues(costAllocationsField) || [];
  ...
  const removeAllocation = useCallback$d(
    (id) => {
      const currentAllocations = getValues(costAllocationsField) || [];
  ...
  const getSelectedValue = useCallback$d(
...
const { useCallback: useCallback$c, useEffect: useEffect$b, useRef: useRef$8 } = await importShared("react");
const useAllocationSync = ({
  ...
  const disableEqualSplit = useDisableEqualSplit(setValue, getValues, isEqualSplitField);
  const setAmountEditing = useCallback$c((editing) => {
    isEditingAmountRef.current = editing;
  }, []);
  const setPercentageEditing = useCallback$c((editing) => {
    isEditingPercentageRef.current = editing;
  }, []);
  const syncPercentageFromAmount = useCallback$c(
    (amount) => {
  ...
  const syncAmountFromPercentage = useCallback$c(
    (percentage) => {
...
const { useCallback: useCallback$b, useEffect: useEffect$a, useMemo: useMemo$c } = await importShared("react");
const useEqualSplit = ({
  allocations,
  isEqualSplit,
  setValue,
  ...
  const distributeEqually = useCallback$b(() => {
    const currentAllocations = getValues(costAllocationsField) || [];
  ...
  const toggleEqualSplit = useCallback$b(() => {
...
const useAmountAllocationSync = ({
  ...
  const prevAmountRef = useRef$7(amount);
...
const canSaveDraft$1 = (data) => {
  const expenseType = data[ExpenseFormField.ExpenseType];
  return !!(expenseType && expenseType.trim() !== "");
};
...
const { useCallback: useCallback$a, useMemo: useMemo$b } = await importShared("react");
...
const { useCallback: useCallback$9, useEffect: useEffect$8, useMemo: useMemo$a, useRef: useRef$6 } = await importShared("react");
const { useCallback: useCallback$8, useMemo: useMemo$9 } = await importShared("react");
const useExpenseDetailsHandlers = (setValue) => {
  const handleCurrencyModeChange = useCallback$8(
    (isDifferent) => {
      setValue("isDifferentCurrency", isDifferent, { shouldValidate: true });
    },
    [setValue]
  );
  const handleNetCurrencyChange = useCallback$8(
    (currency) => {
      setValue("netCurrency", currency, { shouldValidate: true });
    },
    [setValue]
  );
  const handleTotalCurrencyChange = useCallback$8(
    (currency) => {
...
const { useCallback: useCallback$7, useImperativeHandle, useRef: useRef$5, useEffect: useEffect$7 } = await importShared("react");
const useFormImperativeHandle = ({
  ...
  const handleFormSubmit = useCallback$7(async () => {
    var _a;
  ...
  const handleDraftSave = useCallback$7(async () => {
...
const { useEffect: useEffect$6, useRef: useRef$4 } = await importShared("react");
...
const useFileHandlers = (options) => {
  ...
  const handleReceiptChange = useCallback$6(
    (attachment) => {
      setValue(
  ...
  const handleSupportingFilesChange = useCallback$6(
    (attachments) => {
...
export {
  ...
  mileageTypeFieldOptional as A,
  costAllocationTypeSchema as B,
  ConfirmDialog as C,
  ...
  EXPENSE_LOCATION_OPTIONS as O,
  useFormButtonStateSync as P,
  useFormImperativeHandle as Q,
  FormSectionProvider as R,
  SupportingFiles as S,
  FormRenderer as T,
  affidavitExpenseValidationStrategy as U,
  VENDOR_OPTIONS as V,
  ReceiptUpload as W,
  useAutoSave as X,
  ...
  useEqualSplit as a8,
  COST_ALLOCATION_LABELS as a9,
  useCostAllocation as aa,
  AllocationTypeChips as ab,
  CostAllocationField as ac,
  AddAllocationExpandable as ad,
  MAX_SUPPORTING_FILES_FOR_MILEAGE_PERIOD as ae,
  expenseDetailsSchema as af,
  expenseJustificationSchema as ag,
  MileageTripFormField as ah,
  AllowedMimeType as ai,
  MIME_TYPE_CONFIG as aj,
  useReimbursableAmountSync as b,
  useAmountAllocationSync as c,
  useReceiptCheckboxEffects as d,
  useExpenseFormLeftColumn as e,
  useExpenseFormHandlers as f,
  fullExpenseValidationStrategy as g,
  ...
  createValidationStrategy as o,
  additionalCommentsFieldOptional as p,
  supportingFilesField as q,
  expenseDescriptionFieldOptional as r,
  supportingFilesSchema as s,
  businessPurposeFieldOptional as t,
  useFormFieldValues as u,
  reimbursableAmountFieldOptional as v,
  rateUnitFieldOptional as w,
  ratePerUnitFieldOptional as x,
  totalDistanceFieldOptional as y,
  expensePeriodFieldOptional as z
};
