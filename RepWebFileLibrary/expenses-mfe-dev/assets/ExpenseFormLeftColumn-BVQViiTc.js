...
import { s as string, o as object, b as boolean, f as custom, a as array, g as date, u as unknown, n as number, l as literal, c as useForm, d as a, e as useWatch, C as Controller, h as useFormState } from "./zod-0mrSG730.js";
import { importShared } from "./__federation_fn_import-DYyjX-5O.js";
import { j as jsxRuntimeExports } from "./jsx-runtime-TULtkvNU.js";
import { V as fs, X as ps, Z as hs, $ as vs, a0 as bs, a1 as xs, P as Pe, _ as _n, i as is, T as Pa, D as Da, l as ls, Y as Yn, a4 as ss, b as cs, a5 as Dt, a6 as ka, H as Ha, N as Nn, F as Fn, I as In, a7 as Vs, a8 as ks, a9 as Ts, L as Ls } from "./index.es-D1zjfnnK.js";
import { E as ECostAllocation, F as FILE_ENDPOINTS } from "./cost-allocation-Lb8etbcc.js";
import { I as Icon } from "./Icon-0rSrBywh.js";
import { a as apiClient, c as create, d as devtools } from "./axiosInstance-BOw_H5u4.js";
import { P as Plus } from "./plus-Z0bsf9tU.js";
import { c as useDebouncedCallback, b as devWarn, d as devError, a as devLog } from "./use-debounced-callback-DQfTK-qN.js";
import { _ as __vitePreload } from "./preload-helper-e_IRvegh.js";
import { u as useCompanyStore } from "./query-keys-BTZcEQIj.js";
const { useCallback: useCallback$i, useEffect: useEffect$e, useRef: useRef$b, useState: useState$5 } = await importShared("react");
...
const ConfirmDialog = ({
  ...
  const [wasOpen, setWasOpen] = useState$5(open);
  const onCloseCompleteRef = useRef$b(onCloseComplete);
  const closeCompleteFiredRef = useRef$b(false);
  useEffect$e(() => {
    ...
    ps,
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(hs, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(vs, { className: "text-base font-semibold text-exp-neutral-600", children: title }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(bs, { className: "text-sm font-normal text-exp-grey-600", children: description })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs(xs, { className: "gap-3 sm:gap-3", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            Pe,
            {
              ...
              children: isLoading ? /* @__PURE__ */ jsxRuntimeExports.jsx(_n, { className: "size-5" }) : confirmLabel
            }
          )
        ] })
      ]
...
const { useCallback: useCallback$h, useEffect: useEffect$d, useMemo: useMemo$g, useRef: useRef$a } = await importShared("react");
...
const { useCallback: useCallback$d, useEffect: useEffect$b, useMemo: useMemo$d } = await importShared("react");
const useEqualSplit = ({
  allocations,
  isEqualSplit,
  setValue,
...
const { useCallback: useCallback$c, useMemo: useMemo$c } = await importShared("react");
...
const { useCallback: useCallback$b, useEffect: useEffect$9, useMemo: useMemo$b, useRef: useRef$7 } = await importShared("react");
const { useCallback: useCallback$a, useMemo: useMemo$a } = await importShared("react");
...
const { useCallback: useCallback$7, useRef: useRef$4 } = await importShared("react");
...
const useAutoSave = (options) => {
  ...
  const performAutoSave = useCallback$7(() => {
...
const { useEffect: useEffect$5, useMemo: useMemo$8 } = await importShared("react");
...
const { useMemo: useMemo$5, useState: useState$4 } = await importShared("react");
...
const { useCallback: useCallback$5, useEffect: useEffect$3, useMemo: useMemo$3, useRef: useRef$3 } = await importShared("react");
function useBlobUrlManager(options = {}) {
  const { shouldKeepUrl } = options;
  const blobUrlsRef = useRef$3(/* @__PURE__ */ new Set());
  const abortControllerRef = useRef$3(null);
  const shouldKeepUrlRef = useRef$3(shouldKeepUrl);
  shouldKeepUrlRef.current = shouldKeepUrl;
  const trackUrl = useCallback$5((url) => {
    blobUrlsRef.current.add(url);
  }, []);
  const revokeUrl = useCallback$5((url) => {
  ...
  const isOwned = useCallback$5((url) => {
    return blobUrlsRef.current.has(url);
  }, []);
  const getAbortSignal = useCallback$5(() => {
    var _a;
  ...
  return useMemo$3(
    ...
    [trackUrl, revokeUrl, isOwned, getAbortSignal]
  );
}
  ...
  try {
    ...
    if (isApiError(error)) {
      ...
      switch (status) {
        ...
        case 503:
        case 504:
          if (retryCount < FILE_UPLOAD_MAX_RETRIES) {
            const delay = calculateExponentialBackoff(retryCount);
            await sleep(delay);
...
const usePendingUploadStore = create()(
  devtools(
    (set, get) => ({
      ...
      setUploadedAttachment: (attachment) => set({
        uploadedAttachment: attachment,
        uploadStatus: "complete"
      }),
      clearUploadedAttachment: () => set({
        uploadedAttachment: null,
        uploadStatus: "idle"
      }),
...
const ReceiptUpload = ({
  ...
  const initialReceiptRef = useRef$2(initialReceipt);
  const attachmentRef = useRef$2(void 0);
  ...
  const [attachment, setAttachment] = useState$3(effectiveInitialReceipt);
  const [localError, setLocalError] = useState$3();
  const [dragActive, setDragActive] = useState$3(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState$3(false);
  const [isDeleting, setIsDeleting] = useState$3(false);
  const [deleteError, setDeleteError] = useState$3(null);
  const [isLoadingFile, setIsLoadingFile] = useState$3(false);
  const [fileLoadError, setFileLoadError] = useState$3(null);
  const [loadedBlobUrl, setLoadedBlobUrl] = useState$3(null);
  const fileLoadAttemptedRef = useRef$2(/* @__PURE__ */ new Set());
  const handleFileUploadInternalRef = useRef$2(null);
  ...
  const loadFile = useCallback$4(async (targetAttachment) => {
    ...
    try {
      const blobUrl = await downloadFile(targetAttachment.id, signal);
  ...
  const handleFileUploadInternal = useCallback$4(async (file, targetExpenseId) => {
    ...
    try {
      ...
      if (effectiveExpenseId && (selectedCompany == null ? void 0 : selectedCompany.shortName)) {
        ...
        uploadResponse = await uploadReceiptFile(file, (progress) => {
          devLog(`Receipt upload: ${progress.percentage}% (${progress.loaded}/${progress.total} bytes)`);
        });
      }
      const newAttachment = createSafeFileMetadata(file, uploadResponse);
      if (uploadResponse.blobUrl) {
        blobManager.trackUrl(uploadResponse.blobUrl);
      }
  ...
  }, [handleError, onReceiptChange, onUploadingChange, expenseId, selectedCompany == null ? void 0 : selectedCompany.shortName, setUploadedAttachment, setUploadStatus, blobManager]);
  ...
  }, [expenseId, handleFileUploadInternal, handleError, onSaveDraftForUpload, startUploadFlow, updateDraftId, setPendingFile, clearPendingFile, setUploadStatus, blobManager]);
  ...
  }, [attachment, disabled, onReceiptChange, clearUploadedAttachment, loadedBlobUrl, blobManager]);
  ...
  const renderSavingDraftState = () => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full items-center justify-center p-8 text-center gap-4", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-4.5 flex items-center justify-center bg-exp-teal-200 rounded-full", children: /* @__PURE__ */ jsxRuntimeExports.jsx(_n, { className: "size-14" }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-exp-grey-600 font-normal", children: "Saving expense..." })
  ] });
  ...
  const renderLoadingFileState = () => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full items-center justify-center p-8 text-center gap-4", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-4.5 flex items-center justify-center bg-exp-teal-200 rounded-full", children: /* @__PURE__ */ jsxRuntimeExports.jsx(_n, { className: "size-14" }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-exp-grey-600 font-normal", children: "Loading receipt..." })
  ] });
  ...
  const renderContent = () => {
    if (uploadStatus === "saving-draft") return renderSavingDraftState();
    if (uploadStatus === "uploading") return renderUploadingState();
    if (localError) return renderErrorState();
    if (fileLoadError) return renderFileLoadErrorState();
    if (isLoadingFile) return renderLoadingFileState();
...
const { Fragment, memo: memo$2, useCallback: useCallback$3, useContext, useMemo: useMemo$2, useState: useState$2 } = await importShared("react");
  ...
  if (section.hideHeader) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(
      ConditionalSection,
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { "data-section-id": section.id, children: /* @__PURE__ */ jsxRuntimeExports.jsx(is, { className: "gap-0 flex", children: /* @__PURE__ */ jsxRuntimeExports.jsx(cs, { className: "p-0", children: section.render(finalProps) }) }) }, section.id)
      }
    );
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    ConditionalSection,
    {
      ...
      behavior,
      onDisabledChange: handleDisabledChange,
      children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { "data-section-id": section.id, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(is, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(ls, { children: headerActionsNode ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            Yn,
            {
              title: section.title,
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(cs, { children: section.render(finalProps) })
      ] }) }, section.id)
    }
  );
};
...
const AllocationTypeChips = memo$1(({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: allocationTypes.map((allocationType) => /* @__PURE__ */ jsxRuntimeExports.jsxs(Nn, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Fn, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      Dt,
      {
        disabled,
...
const { memo, useState: useState$1, useCallback: useCallback$2 } = await importShared("react");
const AddAllocationExpandable = memo(({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(Vs, { open: isOpen, onOpenChange: setIsOpen, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(ks, { asChild: true, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Pe, { variant: "outlined", icon: /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, {}), disabled, children: COST_ALLOCATION_LABELS.ADD_ALLOCATION }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(Ts, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(AllocationTypeChips, { onSelect: handleTypeSelect, disabled }) })
  ] });
});
...
function FileCard({
  ...
  if (isUploading) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(is, { className: "bg-white px-2 py-2 w-full flex justify-between flex-row items-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(_n, { className: "size-4" }),
  ...
  if (hasError) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(is, { className: "bg-exp-red-100 px-2 py-2 w-full flex justify-between flex-row items-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center flex-1 min-w-0 gap-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(Nn, { delayDuration: FILE_NAME_POPOVER_DELAY, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Fn, { className: "min-w-0 text-left flex flex-nowrap items-center", children: fileName(file.originalName) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(In, { variant: "light", size: "sm", className: "max-w-full", side: "bottom", showArrow: false, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-black", children: file.originalName }) })
        ] })
      ] }),
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(is, { className: "bg-white hover:bg-trax-neutral-20 px-2 py-2 w-full flex justify-between flex-row items-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex min-w-0 flex-1 overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Nn, { delayDuration: FILE_NAME_POPOVER_DELAY, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Fn, { className: "min-w-0 text-left flex flex-nowrap items-center", children: fileName(file.originalName) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(In, { variant: "light", size: "sm", className: "max-w-full", side: "bottom", showArrow: false, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm", children: file.originalName }) })
        ] }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
...
function SupportingFiles({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(ls, { className: `flex items-center flex-col w-full h-fit overflow-clip ${className}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-row w-full items-center", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center w-full ml-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Nn, { delayDuration: 120, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Fn, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(GrCircleInformation, { className: "text-gray-500 hover:cursor-pointer select-none" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(In, { variant: "default", side: "top", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-trax-blue-700 text-white border-none text-sm p-2 w-64", children: displayTooltip }) })
      ] }) }),
      ...
      ) : !hideAddButtonWhenFull ? /* @__PURE__ */ jsxRuntimeExports.jsxs(Nn, { delayDuration: 120, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Fn, { asChild: true, children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleAddSupportingDocumentClick, disabled: true, children: /* @__PURE__ */ jsxRuntimeExports.jsx(FaPlus, { className: "opacity-20 hover:cursor-not-allowed" }) }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          In,
          {
            variant: "light",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex w-full", children: /* @__PURE__ */ jsxRuntimeExports.jsx(is, { className: `bg-white w-full p-0 ${state.attachments.length === 0 && !state.isUploading ? "h-0" : "h-auto"}`, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col my-1 mx-2 gap-1", children: [
      ...
      state.isUploading && state.uploadingFileIndex !== void 0 && state.uploadingFileIndex >= state.attachments.length && /* @__PURE__ */ jsxRuntimeExports.jsx(is, { className: "bg-white px-2 py-2 w-full flex justify-between flex-row items-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(_n, { className: "size-4" }),
...
export {
  ...
  resolveFileUrl as I,
  isMileageTripData as J,
  isMileagePeriodData as K,
  usePendingUploadStore as L,
  MileageFormType as M,
  minimalExpenseValidationStrategy as N,
  useBaseExpenseForm as O,
  EXPENSE_LOCATION_OPTIONS as P,
  useFormButtonStateSync as Q,
  ReceiptSection as R,
  useFormImperativeHandle as S,
  FormSectionProvider as T,
  FormRenderer as U,
  VENDOR_OPTIONS as V,
  affidavitExpenseValidationStrategy as W,
  useAutoSave as X,
  BaseExpenseFormRenderer as Y,
  expenseDescriptionField as Z,
  businessPurposeField as _,
  useMileageRateSync as a,
  rateUnitField as a0,
  ratePerUnitField as a1,
  totalDistanceField as a2,
  expensePeriodField as a3,
  mileageTypeField as a4,
  CostAllocationValidationRules as a5,
  PAYMENT_METHOD_DATA as a6,
  CURRENCY_OPTIONS as a7,
  useCostAllocation as a8,
  AllocationTypeChips as a9,
  CostAllocationField as aa,
  AddAllocationExpandable as ab,
  useEqualSplit as ac,
  COST_ALLOCATION_LABELS as ad,
  SupportingFiles as ae,
  MAX_SUPPORTING_FILES_FOR_MILEAGE_PERIOD as af,
  expenseDetailsSchema as ag,
  expenseJustificationSchema as ah,
  MileageTripFormField as ai,
  AllowedMimeType as aj,
  MIME_TYPE_CONFIG as ak,
  useReimbursableAmountSync as b,
  useAmountAllocationSync as c,
  useReceiptCheckboxEffects as d,
  useExpenseFormHandlers as e,
