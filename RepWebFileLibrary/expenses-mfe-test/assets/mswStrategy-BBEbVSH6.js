...
import { am as AllowedMimeType, an as MIME_TYPE_CONFIG } from "./ExpenseFormLeftColumn-CqC5ralh.js";
import { E as ENDPOINT_PATTERNS, a as addUploadedFile, g as getUploadedFile, h as hasUploadedFile, d as deleteUploadedFile, s as shouldSimulateError, b as generateMockError, c as getExpenseSubmitted, e as getExpenseDrafts, f as getMileageSubmitted, i as getMileageDrafts, m as mockLogicalCompanies, j as mockFormTypes, k as getExpenseTypes, l as getNextExpenseTypeId, n as addExpenseType, u as updateExpenseType, o as getExpenseType, p as getMileageRates, r as recalculateAllRateStatuses, q as sortRatesByStatus, t as getNextMileageRateId, v as setMileageRates, w as calculateRateStatus, x as mapMockToExpenseFormResponse, y as findItemById, z as shouldMockEndpoint } from "./expense-response-mappers-B3Kf4RJi.js";
import "./mileage-trip-sections-Bz7Bx2Rr.js";
import "./jsx-runtime-TULtkvNU.js";
import "./index.es-fHErn31W.js";
import "./Icon-BxYHdB7c.js";
import "./currency-DF2IpVR4.js";
import { a as devLog, b as devWarn, d as devError } from "./use-debounced-callback-DXpXujA5.js";
...
const createDraftHandlers = (config) => {
  ...
  const drafts = ((_a3 = config.externalStore) == null ? void 0 : _a3.drafts) ?? /* @__PURE__ */ new Map();
  const submitted = ((_b3 = config.externalStore) == null ? void 0 : _b3.submitted) ?? /* @__PURE__ */ new Map();
  if (!config.externalStore && config.initialDrafts) {
    config.initialDrafts.forEach((draft) => drafts.set(draft.id, draft));
  }
  if (!config.externalStore && config.initialSubmitted) {
    config.initialSubmitted.forEach((item) => submitted.set(item.id, item));
  }
  const delays = {
    save: ((_c3 = config.delays) == null ? void 0 : _c3.save) ?? 600,
    update: ((_d2 = config.delays) == null ? void 0 : _d2.update) ?? 600,
    get: ((_e2 = config.delays) == null ? void 0 : _e2.get) ?? 300,
    delete: ((_f2 = config.delays) == null ? void 0 : _f2.delete) ?? 500,
    submit: ((_g2 = config.delays) == null ? void 0 : _g2.submit) ?? 1e3
  };
  return [
...
const expenseHandlers = createDraftHandlers({
  ...
  externalStore: {
    drafts: getExpenseDrafts(),
    submitted: getExpenseSubmitted()
  }
});
const mileageHandlers = createDraftHandlers({
  ...
  externalStore: {
    drafts: getMileageDrafts(),
    submitted: getMileageSubmitted()
  }
});
const configurationHandlers = [
  ...
  http.get(ENDPOINT_PATTERNS.CONFIG_LOGICAL_COMPANIES, async () => {
    ...
    console.log("✅ MSW: Logical companies fetched", { count: mockLogicalCompanies.length });
    return HttpResponse.json(response, { status: 200 });
  }),
  // Form Types
  http.get(ENDPOINT_PATTERNS.CONFIG_FORM_TYPES, async () => {
    await delay(300);
    console.log("✅ MSW: Form types fetched", { count: mockFormTypes.length });
  ...
  http.post(ENDPOINT_PATTERNS.CONFIG_EXPENSE_TYPE_CRUD, async ({ request, params }) => {
    ...
    try {
      ...
      console.log("✅ MSW: Expense type created", newExpenseType);
  ...
  http.get(ENDPOINT_PATTERNS.CONFIG_MILEAGE_RATES, async ({ params }) => {
    ...
    try {
      ...
      console.log("✅ MSW: Mileage rates fetched", { company: companyShortName, expenseTypeId, count: rates.length });
      return HttpResponse.json(response, { status: 200 });
    } catch {
      return HttpResponse.json(
    ...
    try {
      ...
      rates = rates.map((rate) => {
        ...
        if (!rate.expiryDate && rateEffective < newEffectiveDate) {
          return {
            ...rate,
            expiryDate: previousMonthEnd.toISOString()
          };
        }
        return rate;
      });
      ...
      console.log("✅ MSW: Mileage rate created", newRate);
  ...
  http.delete(ENDPOINT_PATTERNS.CONFIG_MILEAGE_RATE_DELETE, async ({ params }) => {
    ...
    try {
      ...
      const currentRate = rates.find((r) => {
        const status = calculateRateStatus(r.effectiveDate, r.expiryDate, isExpenseTypeActive);
        return status === MileageRateStatus.CURRENT;
      });
      if (currentRate && currentRate.expiryDate) {
        const hasLaterRate = rates.some((r) => {
          const rEffective = new Date(r.effectiveDate);
          const currentEffective = new Date(currentRate.effectiveDate);
          return rEffective > currentEffective;
        });
        if (!hasLaterRate) {
          currentRate.expiryDate = null;
        }
      }
      rates = recalculateAllRateStatuses(rates, isExpenseTypeActive);
      rates = sortRatesByStatus(rates);
      ...
      console.log("✅ MSW: Mileage rate deleted", { rateId });
      return HttpResponse.json(response, { status: 200 });
    } catch {
      return HttpResponse.json(
...
const expensesListHandlers = [
  ...
  http.get(ENDPOINT_PATTERNS.EXPENSE_FORMS_LIST, async ({ params }) => {
    ...
    console.log("✅ MSW: Expense forms list fetched", { company, count: all.length });
    return HttpResponse.json(response, { status: 200 });
  }),
