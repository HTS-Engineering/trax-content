...
import { u as useMutation } from "./useMutation-C5Eo697z.js";
...
function mapFormDataToCreateRequest(data, expenseTypes) {
  ...
  if (typeId === null) {
    throw new Error("expenseType is required for draft save");
  }
  const selectedExpenseType = expenseTypes == null ? void 0 : expenseTypes.find(
    (et) => parseInt(et.id, 10) === typeId
  );
  if (!(selectedExpenseType == null ? void 0 : selectedExpenseType.formTypeId)) {
    throw new Error("Could not determine formTypeId for selected expense type");
  }
  return {
    typeId,
    formTypeId: selectedExpenseType.formTypeId,
...
function useExpenseItemMutations(options) {
  ...
  const handleExpenseSubmit = useCallback$1(async (data) => {
    try {
      ...
      if (!draftId) {
        const { draft } = await saveExpenseDraftMutation.mutateAsync({ data });
        draftId = draft.id;
      } else {
        await saveExpenseDraftMutation.mutateAsync({ data, draftId });
      }
