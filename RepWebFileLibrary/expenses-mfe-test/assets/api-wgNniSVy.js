import { l as useQuery, t as useQueryClient, q as queryKeys } from "./query-keys-Bur_m63F.js";
...
const mapBackendToFrontend = (backend, queryClient) => {
  ...
  return {
    ...
    // For mileage types: get current rate from mileageEffectiveRate
...
const mapFrontendToBackendCreate = (frontend, queryClient) => {
  const isMileage = frontend.formType === "mileage";
  return {
    ...
    // TODO: Backend needs to add taxTypeId support - see TRX-160 defect
...
const useExpenseTypes = (companyShortName, optionsOrIncludeInactive = false) => {
  ...
  const { includeInactive = false, formTypeIds } = options;
  return useQuery({
    queryKey: companyShortName ? [...queryKeys.expenseTypes.list(companyShortName, includeInactive), { formTypeIds }] : queryKeys.expenseTypes.lists(),
    queryFn: async () => {
...
const useToggleExpenseTypeStatus = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_updatedExpenseType, variables) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.expenseTypes.all()
      });
      queryClient.invalidateQueries({
        queryKey: queryKeys.mileageRates.byExpenseType(variables.companyShortName, parseInt(variables.id, 10))
      });
    }
  });
};
