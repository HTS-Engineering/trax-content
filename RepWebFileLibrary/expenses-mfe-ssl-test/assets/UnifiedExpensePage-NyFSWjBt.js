...
import { x as xs, f as bs, g as ws, S as Ss, C as Cs, y as ys, $ as $e, E as En, W as Ws, c as qa, Y as Ya, d as Wa, K as Ka, M as Mn, P as Pn, V as Vn, B as Bs, a as $a, h as Va, k as ka, J as Jn, G as Ga, T as Ta, i as cs, j as gs, Q as Qn, u as us, p as ps, z as zt, l as Ba, L as Ls, H as Hs, O as Os, Z as Zn, m as za, n as Ks, o as Te, r as Za } from "./configuration-EGHx9r2n.js";
import { E as ExpensesList } from "./ExpensesList-DGf2EE0z.js";
import { c as create, d as devtools, a as apiClient } from "./axiosInstance-BTNM_CBw.js";
import { A as ECostAllocation, B as useQueryClient, i as useCompanyStore, C as useMutation, q as queryKeys, E as EXPENSE_ENDPOINTS, I as ItemCategory, o as useQuery, D as DEFAULT_CURRENCY_CODE, F as FormTypeId, G as useCountries, H as useDefaultCountry, J as ExpenseFormType, K as AllowedMimeType, L as FILE_SIZE_LIMITS, M as MIME_TYPE_CONFIG, P as useDebouncedCallback, S as useDefaultCompany, T as useFormTypeId, U as FilePreviewType, V as FILE_ENDPOINTS, n as useExpenseTypes, W as useCurrencies, X as useTaxTypesDisplay, Y as useDefaultCurrency, t as isRegularExpense, y as isMileageExpense, s as isExpenseItemSubmitted, f as useNavigate, Z as useNavigateBack, z as generatePath, a as RoutePaths, h as useParams, r as useSearchParams, w as useExpenseItem, _ as isExpenseItemDraft } from "./expense-api-CeVpbLJx.js";
import { E as ExpenseStatusBadge } from "./expense-type-badge-B2lVWFfV.js";
...
import { i as isMileageTripData, c as MileageFormType, r as resolveFileUrl, a as isMileagePeriodData, d as formatExpensePeriod, E as ExpensePreview, M as MileageTripPreview, b as MileagePeriodPreview } from "./MileagePeriodPreview-CCVj_M99.js";
import { u as useForm, a, b as useWatch, s as string, D as DECIMAL_FORMAT_REGEX, o as object, c as custom, d as boolean, e as array, f as date, g as unknown, n as number, l as literal, C as Controller, h as createDecimalChangeHandler, i as useBusinessPurposes, j as useFormState } from "./business-purpose-api-C16PvmpV.js";
import { _ as __vitePreload } from "./preload-helper-e_IRvegh.js";
import { I as Icon } from "./Icon-vRMTtv5F.js";
import { P as Plus } from "./EmptyState-B6NFD-Ay.js";
import { b as formatRate, c as formatCurrency, d as formatDate } from "./formatters-aX_vhGGI.js";
const __iconNode$2 = [
...
const { useCallback: useCallback$r, useEffect: useEffect$n, useRef: useRef$k, useState: useState$9 } = await importShared("react");
...
const ConfirmDialog = ({
  ...
  const [wasOpen, setWasOpen] = useState$9(open);
  const onCloseCompleteRef = useRef$k(onCloseComplete);
  const closeCompleteFiredRef = useRef$k(false);
  useEffect$n(() => {
...
const { useEffect: useEffect$m, useRef: useRef$j } = await importShared("react");
const useSetDefaultCurrency = (setValue, defaultCurrencyCode, defaultCurrencySymbol, canSetDefaultCurrency) => {
  const hasSetDefaults = useRef$j(false);
  useEffect$m(() => {
...
const { useCallback: useCallback$q, useEffect: useEffect$l, useMemo: useMemo$n, useRef: useRef$i } = await importShared("react");
function useBaseExpenseForm(config2, props) {
  ...
  const prevIsDraftingRef = useRef$i(isDrafting);
  useEffect$l(() => {
...
const { useCallback: useCallback$n, useEffect: useEffect$k, useRef: useRef$h } = await importShared("react");
const useAllocationSync = ({
  ...
  const isEditingAmountRef = useRef$h(false);
  const isEditingPercentageRef = useRef$h(false);
  const disableEqualSplit = useDisableEqualSplit(setValue, getValues, isEqualSplitField);
...
const { useCallback: useCallback$m, useEffect: useEffect$j, useMemo: useMemo$k } = await importShared("react");
const useEqualSplit = ({
  allocations,
  isEqualSplit,
  setValue,
...
const { useEffect: useEffect$i, useRef: useRef$g } = await importShared("react");
const useAmountAllocationSync = ({
  ...
  const prevAmountRef = useRef$g(amount);
  const setValueRef = useRef$g(setValue);
  const getValuesRef = useRef$g(getValues);
  const triggerRef = useRef$g(trigger);
  useEffect$i(() => {
...
const { useEffect: useEffect$h } = await importShared("react");
...
const { useCallback: useCallback$k, useEffect: useEffect$g, useMemo: useMemo$h, useRef: useRef$f } = await importShared("react");
const { useCallback: useCallback$j, useEffect: useEffect$f, useImperativeHandle, useRef: useRef$e } = await importShared("react");
const useFormImperativeHandle = ({
  ref,
  form,
  onFormErrors
}) => {
  const formRef = useRef$e(form);
  const onFormErrorsRef = useRef$e(onFormErrors);
  useEffect$f(() => {
...
const { useEffect: useEffect$e, useRef: useRef$d } = await importShared("react");
const useFormButtonStateSync = ({
  ...
  const formRef = useRef$d(form);
  const onButtonStateChangeRef = useRef$d(onButtonStateChange);
  useEffect$e(() => {
    formRef.current = form;
...
const useAutoSave = (options) => {
  ...
  const onSaveDraftRef = useRef$c(onSaveDraft);
...
const { useEffect: useEffect$c, useMemo: useMemo$g } = await importShared("react");
...
const { useEffect: useEffect$b, useMemo: useMemo$c } = await importShared("react");
...
const useExpenseFormSync = ({
  ...
  const prevNetAmountRef = useRef$b(netAmount);
  const prevExpenseLocationRef = useRef$b(expenseLocation);
...
const { useMemo: useMemo$a, useState: useState$8 } = await importShared("react");
const useExpenseFormLeftColumn = ({
  ...
  const [isReceiptUploading, setIsReceiptUploading] = useState$8(false);
...
const { useCallback: useCallback$b, useEffect: useEffect$9, useMemo: useMemo$8, useRef: useRef$a } = await importShared("react");
function useBlobUrlManager(options = {}) {
  const { shouldKeepUrl } = options;
  const blobUrlsRef = useRef$a(/* @__PURE__ */ new Set());
  const abortControllerRef = useRef$a(null);
  const batchControllersRef = useRef$a(/* @__PURE__ */ new Set());
  const shouldKeepUrlRef = useRef$a(shouldKeepUrl);
...
const { useEffect: useEffect$8, useMemo: useMemo$6, useRef: useRef$9 } = await importShared("react");
...
const ExpenseDetailsSection = ({
  ...
  const prevShowTaxFields = useRef$9(showTaxFields);
  useEffect$8(() => {
  ...
  const prevShowConvertedTotal = useRef$9(showConvertedTotal);
  useEffect$8(() => {
...
const ExpenseJustificationSection = ({
  ...
  const prevShowPersonsEntertained = useRef$8(showPersonsEntertained);
  useEffect$7(() => {
...
const { useCallback: useCallback$9, useEffect: useEffect$6, useRef: useRef$7, useState: useState$7 } = React$2;
const ReceiptUpload = ({
  onReceiptChange,
  onUploadingChange,
  initialReceipt,
  ...
  const fileInputRef = useRef$7(null);
  const initialReceiptRef = useRef$7(initialReceipt);
  const attachmentRef = useRef$7(void 0);
  const {
    selectedCompany,
    blobManager,
    isStoreDataRelevant,
  ...
  const fileLoadAttemptedRef = useRef$7(/* @__PURE__ */ new Set());
  const handleFileUploadInternalRef = useRef$7(null);
  ...
  const prevAttachmentIdRef = useRef$7(void 0);
  useEffect$6(() => {
...
function ConditionalSection({
  ...
  const previousDisabledRef = useRef$6(void 0);
...
const { Fragment, memo: memo$3, useCallback: useCallback$8, useContext, useMemo: useMemo$5, useState: useState$6 } = await importShared("react");
...
const { memo: memo$1, useCallback: useCallback$7, useState: useState$5 } = await importShared("react");
const AddAllocationExpandable = memo$1(({
  onTypeSelect,
  disabled = false
}) => {
  const [isOpen, setIsOpen] = useState$5(false);
...
const { useEffect: useEffect$4, useRef: useRef$5, useState: useState$4 } = await importShared("react");
...
const { useCallback: useCallback$6, useEffect: useEffect$3, useRef: useRef$4, useState: useState$3 } = await importShared("react");
function SupportingFiles({
  ...
  const attachmentsForCleanupRef = useRef$4([]);
  const {
    selectedCompany,
    blobManager,
    isStoreDataRelevant,
  ...
  const previousAttachmentsRef = useRef$4(effectiveInitialFiles);
  const [loadedBlobUrls, setLoadedBlobUrls] = useState$3(/* @__PURE__ */ new Map());
  const [loadingPreviewFileIds, setLoadingPreviewFileIds] = useState$3(/* @__PURE__ */ new Set());
  const isSavingDraft = supportingFilesUploadStatus === "saving-draft";
  const isUploading = supportingFilesUploadStatus === "uploading";
  const attachmentsRef = useRef$4(effectiveInitialFiles);
  attachmentsRef.current = state.attachments;
  const handleFilesUploadRef = useRef$4(null);
  const previewAbortControllersRef = useRef$4(/* @__PURE__ */ new Map());
  const deletedFileIdsRef = useRef$4(/* @__PURE__ */ new Set());
...
const { memo, useEffect: useEffect$2, useRef: useRef$3 } = await importShared("react");
...
function CostAllocationSectionComponent({
  ...
  const prevCurrencyCodeRef = useRef$3(currency);
  useEffect$2(() => {
