...
import { x as xs, d as bs, e as ws, S as Ss, C as Cs, y as ys, $ as $e, E as En, W as Ws, f as qa, Y as Ya, g as Wa, K as Ka, M as Mn, P as Pn, V as Vn, B as Bs, a as $a, h as Va, k as ka, J as Jn, G as Ga, T as Ta, i as cs, j as gs, Q as Qn, u as us, p as ps, z as zt, l as Ba, L as Ls, H as Hs, O as Os, Z as Zn, m as za, n as Ks, o as Te, b as $r, r as Za } from "./configuration-DhK3YPGc.js";
import { a as ExpenseStatusBadge, E as ExpensesList } from "./ExpensesList-DM0jvWGm.js";
import { c as create, d as devtools, a as apiClient } from "./axiosInstance-BP1HyBRf.js";
import { v as ECostAllocation, w as useQueryClient, i as useCompanyStore, x as useMutation, q as queryKeys, E as EXPENSE_ENDPOINTS, I as ItemCategory, o as useQuery, D as DEFAULT_CURRENCY_CODE, F as FormTypeId, y as useCountries, z as useDefaultCountry, A as ExpenseFormType, B as AllowedMimeType, C as FILE_SIZE_LIMITS, M as MIME_TYPE_CONFIG, G as useDebouncedCallback, H as useDefaultCompany, J as useFormTypeId, K as FilePreviewType, L as FILE_ENDPOINTS, n as useExpenseTypes, P as useCurrencies, S as useTaxTypesDisplay, T as useDefaultCurrency, U as useFormTypeName, V as EMPTY_CURRENCY_SYMBOL, W as isRegularExpense, X as isMileageExpense, Y as isExpenseItemSubmitted, f as useNavigate, Z as useNavigateBack, t as generatePath, a as RoutePaths, h as useParams, r as useSearchParams, _ as useExpenseItem, $ as isExpenseItemDraft } from "./expense-api-BNqyKO6_.js";
...
import { T as Trash2, S as Send } from "./trash-2-DzpZqt7i.js";
import { d as devError, a as devLog, b as devWarn } from "./index-D4vrrc7u.js";
import { u as useForm, a, b as useWatch, s as string, D as DECIMAL_FORMAT_REGEX, o as object, c as custom, d as boolean, e as array, f as date, g as unknown, n as number, l as literal, C as Controller, h as createDecimalChangeHandler, i as useBusinessPurposes, j as useFormState } from "./business-purpose-api-BjN2aURy.js";
import { _ as __vitePreload } from "./preload-helper-e_IRvegh.js";
import { I as Icon } from "./Icon-CWd-YT3m.js";
import { P as Plus } from "./EmptyState-DeDn84y2.js";
import { b as formatRate, c as formatCurrency, d as formatDate, e as formatDistance } from "./formatters-DvYhtky5.js";
import { a as formatExpensePeriod } from "./date-format-D9m87-p-.js";
const __iconNode$2 = [
...
const ConfirmDialog = ({
  ...
  const [wasOpen, setWasOpen] = useState$b(open);
  const onCloseCompleteRef = useRef$m(onCloseComplete);
  const closeCompleteFiredRef = useRef$m(false);
  useEffect$p(() => {
...
function dateToISOString(date2) {
...
const { useEffect: useEffect$o, useRef: useRef$l } = await importShared("react");
const useSetDefaultCurrency = (setValue, defaultCurrencyCode, defaultCurrencySymbol, canSetDefaultCurrency) => {
  const hasSetDefaults = useRef$l(false);
  useEffect$o(() => {
...
function useBaseExpenseForm(config2, props) {
  ...
  const prevIsDraftingRef = useRef$k(isDrafting);
  useEffect$n(() => {
  ...
  const fieldsForValidation = useMemo$n(() => {
  ...
  const formDataForValidation = useMemo$n(() => {
  ...
  const validationErrors = useMemo$n(() => {
    const errorMessages = [];
...
const useCostAllocation = ({
  ...
  const remainingPercentage = useMemo$m(() => {
    return Math.max(0, 100 - allocatedPercentage);
  }, [allocatedPercentage]);
  const progressValue = useMemo$m(() => {
    return parseFloat(allocatedPercentage.toFixed(2));
  }, [allocatedPercentage]);
  const progressError = useMemo$m(() => {
...
const useEqualSplit = ({
  ...
  const canEnableEqualSplit = useMemo$k(() => {
  ...
  const shouldDisableEqualSplit = useMemo$k(() => {
...
const { useEffect: useEffect$k, useRef: useRef$i } = await importShared("react");
const useAmountAllocationSync = ({
  ...
  const prevAmountRef = useRef$i(amount);
  const setValueRef = useRef$i(setValue);
  const getValuesRef = useRef$i(getValues);
  const triggerRef = useRef$i(trigger);
  useEffect$k(() => {
...
const fullExpenseFormSchema = receiptWithSupportingFilesSchema.safeExtend(expenseDetailsSchema.shape).safeExtend(expenseJustificationSchema.shape).safeExtend(costAllocationSchema.shape).safeExtend(additionalCommentsSchema.shape).safeExtend({
  ...
  var _a, _b;
  if (((_a = data.netCurrency) == null ? void 0 : _a.code) !== ((_b = data.totalCurrency) == null ? void 0 : _b.code)) {
  ...
  const isConverted = ((_a = data.netCurrency) == null ? void 0 : _a.code) !== ((_b = data.totalCurrency) == null ? void 0 : _b.code);
...
const { useEffect: useEffect$j } = await importShared("react");
...
const useFormImperativeHandle = ({
  ...
  const formRef = useRef$g(form);
  const onFormErrorsRef = useRef$g(onFormErrors);
  useEffect$h(() => {
...
const { useEffect: useEffect$g, useRef: useRef$f } = await importShared("react");
const useFormButtonStateSync = ({
  ...
  const onButtonStateChangeRef = useRef$f(onButtonStateChange);
  useEffect$g(() => {
    formRef.current = form;
...
const { useEffect: useEffect$e, useMemo: useMemo$g } = await importShared("react");
const useReimbursableAmountSync = ({
  ...
  const calculatedAmount = useMemo$g(() => {
    try {
...
const { useEffect: useEffect$d, useMemo: useMemo$c } = await importShared("react");
...
const { useEffect: useEffect$c, useRef: useRef$d } = await importShared("react");
const useExpenseFormSync = ({
  ...
  const prevNetAmountRef = useRef$d(netAmount);
  const prevExpenseLocationRef = useRef$d(expenseLocation);
  ...
  const isConverted = (netCurrency == null ? void 0 : netCurrency.code) !== (totalCurrency == null ? void 0 : totalCurrency.code);
...
const { useMemo: useMemo$a, useState: useState$a } = await importShared("react");
const useExpenseFormLeftColumn = ({
  ...
  const [isReceiptUploading, setIsReceiptUploading] = useState$a(false);
  ...
  const shouldShowReceiptCheckbox = useMemo$a(() => {
...
const { useCallback: useCallback$b, useEffect: useEffect$b, useMemo: useMemo$8, useRef: useRef$c } = await importShared("react");
function useBlobUrlManager(options = {}) {
  const { shouldKeepUrl } = options;
  const blobUrlsRef = useRef$c(/* @__PURE__ */ new Set());
  const abortControllerRef = useRef$c(null);
  const batchControllersRef = useRef$c(/* @__PURE__ */ new Set());
  const shouldKeepUrlRef = useRef$c(shouldKeepUrl);
  shouldKeepUrlRef.current = shouldKeepUrl;
  const trackUrl = useCallback$b((url) => {
    blobUrlsRef.current.add(url);
  }, []);
  const revokeUrl = useCallback$b((url) => {
  ...
  const isOwned = useCallback$b((url) => {
    return blobUrlsRef.current.has(url);
  }, []);
  const getAbortSignal = useCallback$b(() => {
    var _a;
    (_a = abortControllerRef.current) == null ? void 0 : _a.abort();
    abortControllerRef.current = new AbortController();
    return abortControllerRef.current.signal;
  }, []);
  const createBatchController = useCallback$b(() => {
...
function useFileOperations(options) {
  ...
  const validateFile = useCallback$a(
    (file) => {
      return validateReceiptFile(file);
    },
    []
  );
  const createAttachmentFromResponse = useCallback$a(
    (file, response) => {
  ...
  const uploadSingleFile = useCallback$a(
  ...
  const deleteFileById = useCallback$a(async (fileId) => {
    await deleteFile(fileId);
  }, []);
  const loadFileContent = useCallback$a(
    async (fileId, signal) => {
  ...
  const openPreview = useCallback$a(
    async (attachment) => {
      try {
...
const { useEffect: useEffect$a, useMemo: useMemo$6, useRef: useRef$b } = await importShared("react");
...
const ExpenseDetailsSection = ({
  ...
  const showConvertedTotal = (netCurrency == null ? void 0 : netCurrency.code) !== (totalCurrency == null ? void 0 : totalCurrency.code);
  ...
  const codeToSymbolMap = useMemo$6(() => {
    return new Map(currencies == null ? void 0 : currencies.map((c) => [c.isoCode, c.symbol]));
  }, [currencies]);
  const prevShowTaxFields = useRef$b(showTaxFields);
  useEffect$a(() => {
  ...
  const prevShowConvertedTotal = useRef$b(showConvertedTotal);
  useEffect$a(() => {
...
const ExpenseJustificationSection = ({
  ...
  const prevShowPersonsEntertained = useRef$a(showPersonsEntertained);
  useEffect$9(() => {
...
const { useCallback: useCallback$9, useEffect: useEffect$8, useRef: useRef$9, useState: useState$9 } = React$2;
const ReceiptUpload = ({
  onReceiptChange,
  onUploadingChange,
  initialReceipt,
  ...
  const initialReceiptRef = useRef$9(initialReceipt);
  const attachmentRef = useRef$9(void 0);
  const {
    selectedCompany,
    blobManager,
    isStoreDataRelevant,
  ...
  const [attachment, setAttachment] = useState$9(effectiveInitialReceipt);
  const [localError, setLocalError] = useState$9();
  const [dragActive, setDragActive] = useState$9(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState$9(false);
  const [isDeleting, setIsDeleting] = useState$9(false);
  const [deleteError, setDeleteError] = useState$9(null);
  const [isLoadingFile, setIsLoadingFile] = useState$9(false);
  const [fileLoadError, setFileLoadError] = useState$9(null);
  const [loadedBlobUrl, setLoadedBlobUrl] = useState$9(null);
  const fileLoadAttemptedRef = useRef$9(/* @__PURE__ */ new Set());
  const handleFileUploadInternalRef = useRef$9(null);
  ...
  const loadFile = useCallback$9(async (targetAttachment) => {
  ...
  const prevAttachmentIdRef = useRef$9(void 0);
  useEffect$8(() => {
  ...
  const handleRetry = useCallback$9(() => {
    if (!attachment) return;
    fileLoadAttemptedRef.current.delete(attachment.id);
    loadFile(attachment);
  }, [attachment, loadFile]);
  const handleError = useCallback$9((error) => {
    setLocalError(error);
    setUploadStatus("error", error.message);
  }, [setUploadStatus]);
  const handleFileUploadInternal = useCallback$9(async (file, targetExpenseId) => {
  ...
  const handleFileUpload = useCallback$9(async (file) => {
  ...
  const handleFileSelect = useCallback$9((event) => {
    var _a;
  ...
  const handleChooseFileClick = useCallback$9(() => {
    var _a;
    if (isUploadDisabled || isProcessing) return;
    (_a = fileInputRef.current) == null ? void 0 : _a.click();
  }, [isUploadDisabled, isProcessing]);
  const handleDeleteClick = useCallback$9(() => {
    if (!attachment || disabled) return;
    setDeleteError(null);
    setIsDeleteDialogOpen(true);
  }, [attachment, disabled]);
  const handleDeleteConfirm = useCallback$9(async () => {
    if (!attachment || disabled) return;
    setIsDeleting(true);
    setDeleteError(null);
    try {
  ...
  const handleDeleteCancel = useCallback$9(() => {
    setIsDeleteDialogOpen(false);
    setDeleteError(null);
  }, []);
  const handlePreviewClick = useCallback$9(async () => {
    if (attachment) {
  ...
  const handleDragOver = useCallback$9((e) => {
    e.preventDefault();
    if (!isUploadDisabled && !isProcessing) {
      setDragActive(true);
    }
  }, [isUploadDisabled, isProcessing]);
  const handleDragLeave = useCallback$9((e) => {
    e.preventDefault();
    setDragActive(false);
  }, []);
  const handleDrop = useCallback$9((e) => {
    e.preventDefault();
    setDragActive(false);
    if (isUploadDisabled || isProcessing) return;
...
function ConditionalSection({
  ...
  const previousDisabledRef = useRef$8(void 0);
...
const { Fragment, memo: memo$3, useCallback: useCallback$8, useContext, useMemo: useMemo$5, useState: useState$8 } = await importShared("react");
const FormSectionComponent = ({
  section,
  handlersMap
}) => {
  const renderProps = useContext(FormSectionContext);
  const [conditionalDisabled, setConditionalDisabled] = useState$8(false);
  if (!renderProps) {
    throw new Error("FormSection must be used within FormSectionProvider");
  }
  const handleDisabledChange = useCallback$8((disabled) => {
      ...
      value && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-1", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-1", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `flex-1 ${inputLabel ? "mt-5" : ""}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          Controller,
          {
            ...
            control,
            render: ({ field }) => (
              //TODO(???): Replace CurrencyInput with AddonInput/FormInput to dynamically render currency symbol
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                Ba,
                {
                  className: "min-w-32",
...
const { memo: memo$1, useCallback: useCallback$7, useState: useState$7 } = await importShared("react");
const AddAllocationExpandable = memo$1(({
  onTypeSelect,
  disabled = false
}) => {
  const [isOpen, setIsOpen] = useState$7(false);
  const handleTypeSelect = useCallback$7(
    (type) => {
      onTypeSelect(type);
      setIsOpen(false);
    },
...
const { useEffect: useEffect$6, useRef: useRef$7, useState: useState$6 } = await importShared("react");
function FileName({ name, showTooltip = true }) {
  const { ref, isTruncated } = Zn({
    deps: [name]
  });
  const [isOpen, setIsOpen] = useState$6(false);
  const timeoutRef = useRef$7(null);
...
const { useCallback: useCallback$6, useEffect: useEffect$5, useRef: useRef$6, useState: useState$5 } = await importShared("react");
function SupportingFiles({
  ...
  const attachmentsForCleanupRef = useRef$6([]);
  const {
    selectedCompany,
    blobManager,
    isStoreDataRelevant,
  ...
  const [deleteDialogState, setDeleteDialogState] = useState$5({
    isOpen: false,
    fileIndex: null,
    isDeleting: false
  });
  const previousAttachmentsRef = useRef$6(effectiveInitialFiles);
  const [loadedBlobUrls, setLoadedBlobUrls] = useState$5(/* @__PURE__ */ new Map());
  const [loadingPreviewFileIds, setLoadingPreviewFileIds] = useState$5(/* @__PURE__ */ new Set());
  const isSavingDraft = supportingFilesUploadStatus === "saving-draft";
  const isUploading = supportingFilesUploadStatus === "uploading";
  const attachmentsRef = useRef$6(effectiveInitialFiles);
  attachmentsRef.current = state.attachments;
  const handleFilesUploadRef = useRef$6(null);
  const previewAbortControllersRef = useRef$6(/* @__PURE__ */ new Map());
  const deletedFileIdsRef = useRef$6(/* @__PURE__ */ new Set());
  ...
  const handleAddSupportingDocumentClick = useCallback$6((e) => {
    var _a;
    e.preventDefault();
    if (disabled || state.isUploading || isSavingDraft) return;
    (_a = fileInputRef.current) == null ? void 0 : _a.click();
  }, [disabled, state.isUploading, isSavingDraft]);
  const handleFilesUpload = useCallback$6(async (filesToProcess, targetExpenseId) => {
    if (!(selectedCompany == null ? void 0 : selectedCompany.shortName)) {
  ...
  const handleFileChange = useCallback$6(async (e) => {
  ...
  const handleRemoveFile = useCallback$6((index) => {
  ...
  const handleDeleteConfirm = useCallback$6(async () => {
  ...
  const handleDeleteCancel = useCallback$6(() => {
    setDeleteDialogState({
      isOpen: false,
      fileIndex: null,
      isDeleting: false
  ...
  const handlePreviewClick = useCallback$6(async (file) => {
    var _a;
...
const { memo, useEffect: useEffect$4, useRef: useRef$5 } = await importShared("react");
function CostAllocationHeaderActions({
  ...
  const allocations = costAllocations || [];
  const isConverted = (netCurrency == null ? void 0 : netCurrency.code) !== (totalCurrency == null ? void 0 : totalCurrency.code);
  const parsedAmount = isConverted ? parseFloat(totalAmount || "0") : parseFloat(netAmount || "0");
...
function CostAllocationSectionComponent({
  ...
  const prevCurrencyCodeRef = useRef$5(currency);
  useEffect$4(() => {
...
const COST_ALLOCATION_SECTION = {
  ...
  isEnabled: (formValues) => {
    var _a, _b;
    const isConverted = ((_a = formValues.netCurrency) == null ? void 0 : _a.code) !== ((_b = formValues.totalCurrency) == null ? void 0 : _b.code);
    const amount = isConverted ? parseFloat(formValues.totalAmount || "0") : parseFloat(formValues.netAmount || "0");
...
const { useCallback: useCallback$3, useRef: useRef$4, useState: useState$4 } = await importShared("react");
...
const { useEffect: useEffect$3, useRef: useRef$3, useState: useState$3 } = await importShared("react");
const PreviewReceiptSection = ({ receipt, stretchToFill = false }) => {
  var _a;
  const [imageError, setImageError] = useState$3(false);
  const [loadedBlobUrl, setLoadedBlobUrl] = useState$3();
  const [isLoading, setIsLoading] = useState$3(false);
  const abortRef = useRef$3(null);
...
const { useEffect: useEffect$2, useRef: useRef$2, useState: useState$2 } = await importShared("react");
...
const PreviewSupportingFilesSection = ({
  ...
  const [loadingFileIds, setLoadingFileIds] = useState$2(/* @__PURE__ */ new Set());
  const [cachedBlobUrls, setCachedBlobUrls] = useState$2(/* @__PURE__ */ new Map());
  const controllersRef = useRef$2(/* @__PURE__ */ new Map());
  useEffect$2(() => {
    return () => {
...
function useUnifiedExpensePage() {
  ...
  const handleOpenChange = useCallback((open) => {
    if (!open) {
      ...
      resetUploadStore();
      if (currentDraftId) {
        queryClient2.removeQueries({ queryKey: queryKeys.expenseItem.detail(currentDraftId) });
      }
      navigateBack();
    }
  }, [navigateBack, resetUploadStore, currentDraftId, queryClient2]);
