...
import { d as apiClient, p, H as Hs, C as Ca, _ as _r, h as On, L as La, i as Ls, j as Sa, R as Ra, S as Sn } from "./axiosInstance-D567fVcX.js";
import { k as useQuery, q as queryKeys } from "./query-keys-CUQuTCZz.js";
import { E as EXPENSE_ENDPOINTS } from "./endpoints-B5IysDN2.js";
import { I as Icon } from "./Icon-5_58Mq6r.js";
import { j as useSearchParams, e as useLocation, u as useNavigate, a as RoutePaths } from "./routes-fA2LM82H.js";
import { C as Clock } from "./clock-Dl8brOyf.js";
...
const STATUS_SPECIFIC_SORT_FIELDS = [STATUS_DATE_COLUMN_ID];
const DEFAULT_SORT_FIELD_ALL = "expenseDate";
const DEFAULT_SORT_ORDER = "desc";
const getDefaultSortField = (status) => status === "all" ? DEFAULT_SORT_FIELD_ALL : STATUS_DATE_COLUMN_ID;
...
const isNavigatingToStatusTab = (prevStatus, newStatus) => prevStatus === "all" && newStatus !== "all";
const useExpensesUrlFilters = () => {
  ...
  useEffect(() => {
    ...
    if (needsSortBy || needsSortOrder || hasStatusSpecificOnAll) {
      setSearchParams((prev) => {
        ...
        next.set(URL_PARAM_SORT_ORDER, DEFAULT_SORT_ORDER);
        return next;
      }, { replace: true });
    }
  }, [isOnListPage, urlSortBy, urlSortOrder, currentStatusFilter, setSearchParams]);
  const sortingRef = useRef([{ id: DEFAULT_SORT_FIELD_ALL, desc: true }]);
  const sorting = useMemo$1(() => {
    ...
    const sortOrder = isValidSortOrder(urlSortOrder) ? urlSortOrder : DEFAULT_SORT_ORDER;
    if (hasStatusSpecificSortOnAll(sortBy, currentStatusFilter)) {
      const result2 = [{ id: DEFAULT_SORT_FIELD_ALL, desc: DEFAULT_SORT_ORDER === "desc" }];
      sortingRef.current = result2;
      return result2;
    }
  ...
  const columnFilters = useMemo$1(() => {
    ...
    const filters = currentStatusFilter === "all" ? [] : [{ id: "status", value: currentStatusFilter }];
    columnFiltersRef.current = filters;
    return filters;
  }, [currentStatusFilter, isOnListPage]);
  ...
  useEffect(() => {
    ...
    setSearchParams((prev) => {
      ...
      if (hasStatusSpecificSortOnAll(currentSortBy, currentStatusFilter)) {
        next.set(URL_PARAM_SORT_BY, DEFAULT_SORT_FIELD_ALL);
        next.set(URL_PARAM_SORT_ORDER, DEFAULT_SORT_ORDER);
      }
      return next;
    }, { replace: true });
  ...
  const handleSortingChange = useCallback$2((updater) => {
    ...
    const sortBy = (sortConfig == null ? void 0 : sortConfig.id) ?? DEFAULT_SORT_FIELD_ALL;
  ...
  const handleColumnFiltersChange = useCallback$2(
    (updater) => {
      ...
      const isNavigatingToStatus = isNavigatingToStatusTab(currentStatusFilter, newStatus);
...
const ExpensesList = () => {
  ...
  const queryParams = useMemo(() => {
    ...
    return {
      ...
      sortBy: mapSortFieldToApi(((_a = sorting[0]) == null ? void 0 : _a.id) ?? "updatedAt", currentStatusFilter),
  ...
  const handleRowClick = useCallback((row) => {
    ...
    const navigationState = { state: { returnUrl } };
    if (expenseType === "mileage") {
      const formType = "formType" in data ? data.formType : "trip";
      if (formType === "period") {
        navigate(`/expenses/mileage-period/${id}`, navigationState);
      } else {
        navigate(`/expenses/mileage-trip/${id}`, navigationState);
      }
    } else {
      navigate(`/expenses/${id}`, navigationState);
    }
