...
import { u as useNavigate, f as useLocation, h as useParams, i as RoutePathConfigChunks } from "./routes-BSabWeNZ.js";
...
const EXPENSE_ENDPOINTS = {
  ...
  MANAGER_DECISION: (company, expenseFormId) => `/api/v1.0/approvals/${company}/${encodeURIComponent(expenseFormId)}/manager-decision`,
...
const mapToLogicalCompany = (apiCompany, userDefaultLogicalCompanyId) => ({
...
function useAdminCompanyUrlBinding() {
  ...
  useEffect$1(() => {
    if (!params.companyId || (adminCompany == null ? void 0 : adminCompany.id) === params.companyId) return;
    const match = companies == null ? void 0 : companies.find((c) => c.id === params.companyId);
    if (match) setAdminCompany(match);
  }, [companies, adminCompany, setAdminCompany, params.companyId]);
  const onCompanyChange = useCallback(
    (companyId) => {
      ...
      const isCompanyScoped = path.includes(`/${RoutePathConfigChunks.ExpenseType}/`) || path.includes(`/${RoutePathConfigChunks.BusinessPurpose}/`);
      if (isCompanyScoped) {
        const segments = path.split("/");
        const baseRoute = segments.slice(0, -1).join("/");
        navigate(`${baseRoute}/${companyId}`);
      }
    },
    [companies, location.pathname, navigate, setAdminCompany]
  );
  return { adminCompany, onCompanyChange };
}
...
export {
  functionalUpdate as $,
  isExpenseItemDraft as A,
  FILE_ENDPOINTS as B,
  useFormTypeName as C,
  DEFAULT_CURRENCY_CODE as D,
  EXPENSE_ENDPOINTS as E,
  FormTypeId as F,
  EMPTY_CURRENCY_SYMBOL as G,
  ECostAllocation as H,
  ItemCategory as I,
  AllowedMimeType as J,
  FILE_SIZE_LIMITS as K,
  FilePreviewType as L,
  MIME_TYPE_CONFIG as M,
  useDefaultCountry as N,
  ...
  matchMutation as T,
  noop as U,
  hashQueryKeyByOptions as V,
  Query as W,
  matchQuery as X,
  focusManager as Y,
  onlineManager as Z,
  resolveStaleTime as _,
  useAdminCompanyUrlBinding as a,
  hashKey as a0,
  partialMatchKey as a1,
  skipToken as a2,
  QueryObserver as a3,
  useBaseQuery as a4,
  ExpenseTypeScope as a5,
  useCreateExpenseType as a6,
  useUpdateExpenseType as a7,
  useToggleExpenseTypeStatus as a8,
  useFormTypeOptions as a9,
  DEFAULT_CURRENCY_SYMBOL as aa,
  MileageRateStatus as ab,
  ensureQueryFn as ac,
  addToStart as ad,
  addToEnd as ae,
  addConsumeAwareSignal as af,
  DEFAULT_CURRENCY as ag,
  formatHistoryTimestamp as ah,
  useCompanyStore as b,
  useFormTypes as c,
  useExpenseTypes as d,
  useQuery as e,
  formatExpenseDate as f,
  isRegularExpense as g,
  getExpenseItemAmount as h,
  isExpenseItemSubmitted as i,
  useQueryClient as j,
  keepPreviousData as k,
  useMutation as l,
  isMileageExpense as m,
  useExpenseItem as n,
  useDefaultCompany as o,
  formatToISODate as p,
  queryKeys as q,
  useCurrencies as r,
  useCountries as s,
  useTaxTypesDisplay as t,
  useCompanies as u,
  useDefaultCurrency as v,
  parseDateOnlyAsLocal as w,
  useFormTypeId as x,
  ExpenseFormType as y,
  formatExpensePeriod as z
};
